...
void init_non_megistos()
{
  ...
  for(i=0,base=0;i<9;i++,base+=(SYS2COD-SYS1COD)){
    ...
    fprintf(stderr,
	    ...
	    non_megistos[i].hostname,non_megistos[i].port,non_megistos[i].bbsd_pid);
#endif
  }
}
...
void register_with_metabbs()
{
  ...
  if(cl==NULL){
    cl=clnt_create(SERVERNAME,METABBS_PROG,METABBS_VERS,"tcp");
    if(cl==NULL){

#ifdef DEBUG
      fprintf(stderr,"Failed to create client! RPC error follows.\n");
  ...
  if(result==NULL){
    ...
    return;
#endif
  } else last_registration_time=time(NULL);
  
#ifdef DEBUG
  if(*result!=0){
    fprintf(stderr,"Failed to register, (daemon error, retval=%d)\n",*result);
  } else {
  ...
  for(i=0;i<9;i++){
    if(non_megistos[i].codename[0]==0)continue;	/* Empty entry */
    ...
    result=metabbs_register_non_megistos_1(&non_megistos[i],cl);
    ...
    if(result==NULL){
      ...
      fprintf(stderr,"\tEntry #%d (%s): Registration ok.\n",i,non_megistos[i].codename);
    }
#endif
