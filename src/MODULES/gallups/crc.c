 ...
 *
 * $Log$
 * Revision 1.1  2001/04/16 14:56:12  alexios
 * Initial revision
 *
 ...
 * adopted from textutils-1.22 file cksum.c
 */
...
unsigned long int cksum (char *buffer, unsigned long int size)
{
  ...
  unsigned char *cp = buffer;
	...
	pos = size;
	while (pos > 0) {
		crc = (crc << 8) ^ crctab[((crc >> 24) ^ pos) & 0xFF];
		pos >>= 8;
	}
