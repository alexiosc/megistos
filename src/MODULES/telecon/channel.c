...
getaccess(char *chan)
{
  if(usr_exists(chan)){
    ...
    if(clubax<=CAX_ZERO)return CHF_PRIVATE;
    else if(clubax<=CAX_READ)return CHF_READONLY;
    else if(clubax<=CAX_UPLOAD)return CHF_OPEN;
    else return CHF_MODERATED;
  }
  return CHF_OPEN;
}
...
enterchannel(char *channel)
{
  ...
  if(sameas(channel,curchannel))return -1;
  ...
  if(new){
    ...
    memcpy(&usr,
	   makechanusr(thisuseracc.userid,
		       getdefusrax(channel,thisuseracc.userid)),
    ...
    setusrax(channel,thisuseracc.userid,thisuseracc.sex,ax,-1);
...
chanscan()
{
  ...
  for(j=0;j<i;j++){
    ...
    if(channels[j]->d_name[0]=='_')channels[j]->d_name[0]='/';
	...
	if(!(usr->flags&CUF_PRESENT)){
	  ...
	  if((sameas(usr->userid,thisuseracc.userid)||
	      sameas(channels[j]->d_name,thisuseracc.userid))
	     &&(usr->flags&CUF_EXPLICIT)){

	    int pr;

	    pr=sameas(channels[j]->d_name,thisuseracc.userid)?SCANAX2:SCANACC;

	    prompt(strcmp(usr->userid,thisuseracc.userid)?SCANTAB1B:SCANTAB1A,
		   usr->userid);
	    ...
	    if(pr==SCANAX2){
	      ...
	      prompt(SCANINVC,channels[j]->d_name);
	    } else prompt(SCANINVO,channels[j]->d_name);
	...
	} else prompt(SCANOTHR,channels[j]->d_name);

	if(usr->flags&CUF_UNLISTED)prompt(SCANUNL);
	if(usr->flags&CUF_READONLY)prompt(SCANRO);

	if(sameas(hdr->moderator,usr->userid)){
	  if(hdr->topic[0])prompt(SCANTPC,hdr->topic);
	}
