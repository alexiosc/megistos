...
showinfo()
{
  ...
  if((c=begscan(curchannel,TSM_PRESENT))==NULL)return;

  if(c->topic[0])prompt(SITOPIC,c->topic);
...
countdown()
{
  if(thisuseronl.telecountdown>0)thisuseronl.telecountdown--;
  if(!thisuseronl.telecountdown)thisuseronl.telecountdown=-2;
}
