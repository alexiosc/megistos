...
{
  int i;
  for(i=0;i<numplugins;i++)if(sameas(plugins[i].keyword,keyword))return 1;
  return 0;
}
...
main(int argc, char *argv[])
{
  ...
  while(!feof(fin)){
	...
	fprintf(stderr,"Line %d: bad format for 'plugin'.\n",num);
	exit(1);
      } else if(pluginexists(tmp)){
	fprintf(stderr,"Line %d: duplicate plugin '%s'.\n",num,tmp);
	exit(1);
      } else {
	int i;
    ...
    case CODE_DESCR:
      {
	int n;
	for(n=0;n<ML&&(p.descr[n][0]!=0);n++);
	if(n>=ML){
	  fprintf(stderr,"Line %d: more than %d 'descr' lines in this def.\n",
		  num,ML);
	  exit(1);
	} else {
