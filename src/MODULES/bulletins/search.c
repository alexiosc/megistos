...
showkeywords()
{
  ...
  if(numkeys==1)prompt(KSCAN1,keywords[0]);
  else {
    prompt(KSCAN2,keywords[0]);
    for(i=1;i<numkeys-1;i++)prompt(KSCAN3,keywords[i]);
...
doscan()
{
  ...
  do{
    ...
    found=0;
    if(keycmp(tmpblt.fname))found=1;
    else if(keycmp(tmpblt.area))found=1;
    else if(keycmp(tmpblt.author))found=1;
    else if(keycmp(tmpblt.descr))found=1;
    else {
      int lines=0;
      FILE *fp;
	...
	while(fgets(line,sizeof(line),fp)){
	  if((lines++)%indspd==0)progressind(lines);
	  if(keycmp(line)){
	    found=2;
	    break;
	  }
