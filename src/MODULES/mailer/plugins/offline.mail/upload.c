...
processmsg()
{
  ...
  if(!setupclub())goto skip;


  /* Setup the recipient of the message */
  
  if(!setuprecipient())goto skip;
  ...
  */
  ...
  if(msg.flags&MSF_RECEIPT)prompt(REPTBRR);
...
process(char *fname)
{
  ...
  while(!feof(rep)){
    int res;
    readbuf();
    if(feof(rep)||ferror(rep))break;
    memcpy(&qwkhdr,&qwkbuf,sizeof(qwkhdr));
    res=processmsg();
    if(res<0)return -1;
    else if(res==1)nummsgs++,numsuccessful++;
    else nummsgs++;
  }
