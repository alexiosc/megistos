...
ctl_request(struct qwkhdr *qwkhdr, struct message *msg)
{
...

  {
    ...
    sprintf(fname,"%s/%s/"MSGATTDIR"/"FILEATTACHMENT,
	    MSGSDIR,att.club[0]?att.club:EMAILDIRNAME,att.msgno);
...
ctl_reset(struct qwkhdr *qwkhdr, struct message *msg)
{
  ...
  if(sameas(omceml,clubname))strcpy(clubname,omceml);
    ...
    if(msgno<0)msgno=max(msgno+clubhdr.msgno,0);
    else msgno=min(msgno,clubhdr.msgno);
    sprintf(tmp,"%s %d",clubname,msgno);
    prompt(DIRRESET,tmp);
