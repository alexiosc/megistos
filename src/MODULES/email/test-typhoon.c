...
main()
{
  ...
  if(j==BSE_END)j=findmsgto(&msgno,LOOKFOR,msgno,2);
  else if(j==BSE_BEGIN)j=findmsgto(&msgno,LOOKFOR,msgno,4);

  for(;db_status==S_OKAY;){
    d_recread(&buf);
    if(strcmp(buf.to,LOOKFOR))break;
    printf("#%2d. %5d. FROM %-10s  TO %-10s  SUBJ %s\n",i++,buf.num,buf.from,buf.to,buf.subject);
    if(bk)d_keyprev(TOC);
    else d_keynext(TOC);
  }
