...
{
  if(numfiles==0)return NULL;	/* Should never happen */
  currentfile=1;
  return &(files[0]);
}
...
{
  if(currentfile>=numfiles)return NULL;
...
static void grow_array()
{
  if(files==NULL||numfiles>=arraysize){
    ...
    if(files!=NULL){
      memcpy(tmp,files,sizeof(struct filerec)*numfiles);
      free(files);
    }
    files=tmp;
  }
...
static void add_file(char *fname,char *summary,int size)
{
  grow_array();
...
idx_expandspec(char *s, int quiet)
{
  ...
  if(strcspn(s,"*?[]")==strlen(s)){
    ...
    if(!quiet)prompt(DNLWAIT);
    if(!mkwildcard(s))return 0;
    for(approved=1;approved>=(grovel?0:1);approved--){
      morefiles=filegetfirst(library.libnum,&f,approved);
      while(morefiles){
	char tmp[24];
	int len=strlen(f.fname);
	strcpy(tmp,f.fname);
	lcase(tmp);
	if(f.approved!=approved || f.flibnum!=library.libnum)break;
	if((len=re_search(regexp,tmp,len,0,len,NULL))>=0){
	  if(first){
	    if(!quiet)prompt(DNLSIH);
	    first=0;
	  }
	  add_file(f.fname,f.summary,size=shdnlinfo(&f,quiet));
	  totalsize+=size;
	  totaltimesec+=calcxfertime(size,1);
	}
...
os_expandspec(char *s,int quiet)
{
  ...
  if(strcspn(s,"*?[]")==strlen(s)){
    ...
    match_to=s;
    if((j=scandir(library.dir,&d,insensitive_match,alphasort))==0){
      if(d)free(d);
      if(!quiet)prompt(DNLNF,s);
      return 0;
    }
	...
	totalcharge+=calccharge(st.st_size,&library);
	...
	size=os_shdnlinfo(d->d_name,osfdesc,quiet);
...
{
  if(library.flags&LBF_OSDIR)return os_expandspec(s,quiet);
