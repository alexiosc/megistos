...
/*
 * $Id: download.c,v 2.0 2004/09/13 19:44:54 alexios Exp $
 *
 * $Log: download.c,v $
...
showmenu ()
{
	...
	if (NUMFILES == 1)
		prompt (DNLHDR, xferlist[firstentry].filename,
			xferlist[firstentry].description, filesize, xfertime,
			msg_getunit (MINSNG, xfertime), filetype);
	else
		prompt (DNLHDR1, filesize, xfertime,
			msg_getunit (MINSNG, xfertime));
...
viewcontents ()
{
	...
	for (i = 0; i < numprotocols; i++) {
		if ((protocols[i].flags & PRF_UPLOAD) == 0 &&
			...
			if (protocols[i].flags & PRF_BINARY)
...
checksuccess (char *fname)
{
	...
	for (i = 0; i < totalitems; i++) {
		if (xferlist[i].flags & XFF_KILLED)
			continue;
		if (xferlist[i].flags & XFF_TAGGED && (!xfertagged))
			continue;
		...
		if ((xferlist[i].flags & (XFF_AUDIT | XFF_AUDITED)) ==
		    XFF_AUDIT) {
	...
	while ((dirent = readdir (dp)) != NULL) {
		if (strcmp (dirent->d_name, ".") &&
			...
			for (i = 0; i < totalitems; i++) {
				if (xferlist[i].
				    flags & (XFF_KILLED /* |XFF_CHECK */ ))
					continue;
				if (xferlist[i].flags & XFF_TAGGED &&
				    (!xfertagged))
					continue;
				if (!strcmp
				    (xferlist[i].filename, dirent->d_name)) {
					if (xferlist[i].auditsfail[0] &&
					    xferlist[i].auditdfail[0]) {
						audit (thisuseronl.channel,
...
download (char *prot)
{
	...
	for (i = 0; i < totalitems; i++) {
		...
		if (xferlist[i].flags & XFF_TAGGED && (!xfertagged))
			continue;
		sprintf (fname, TMPDIR "/dnl%05d/%s", getpid (),
			 xferlist[i].filename);
...

	{
		...
		if (!pid) {
			...
			*/
			...
			error_fatal
			    ("Unable to fork download protocol for some obscure reason.");
		}
...
tageditor ()
{
	...
	while (get_menu (&c, firsttime, TEMENU, TESHMNU, TEERR, "LTDS", 0, 0)) {
		...
		case 'L':
			{
				...
				for (i = 0; i < totalitems; i++) {
					prompt (TELTAB, i + 1,
						xferlist[i].filename,
						xferlist[i].
						flags & XFF_TAGGED ? mark : "",
						xferlist[i].
						flags & XFF_KILLED ? mark : "",
		...
		case 'T':
			if (get_number
			    (&i, TENASK, 1, totalitems, TENERR, 0, 0)) {
				if (xferlist[i - 1].dir == FXM_TRANSIENT &&
				    (xferlist[i - 1].flags & XFF_TAGGED) ==
				    0) {
					cnc_end ();
					prompt (CANTAG,
						xferlist[i - 1].filename);
				} else {
					xferlist[--i].flags ^= XFF_TAGGED;
					prompt (xferlist[i].
						flags & XFF_TAGGED ? TENTAG :
						TENUTG, i + 1,
						xferlist[i].filename);
				}
			}
		...
		case 'D':
			if (get_number
				...
				xferlist[--i].flags ^= XFF_KILLED;
				prompt (xferlist[i].
					flags & XFF_KILLED ? TENKLL : TENUKL,
					i + 1, xferlist[i].filename);
			}
			break;
		case 'S':
			restat ();
			prompt (TESTAT, numitems, filesize, xfertime,
				msg_getunit (MINSNG, xfertime), numtagged,
				taggedsize);
			break;
		}
	}
...
dologout ()
{
	...
	for (;;)
		if (get_bool (&yes, LOGOUT2, 0, 0, 0))
			break;
	if (!yes)
		exit (0);
	if (cnc_nxtcmd == NULL || !*cnc_nxtcmd)
		cnc_end ();
}
...
confirm_exit ()
{
	...
	if (!get_bool (&retval, CONFX, 0, 0, 0))
		return 0;
	return retval;
}
		...
		if (sameas (s, "?"))
			...
			if (sameas (s, "V")) {
				...
				if (!NUMFILES) {
					...
					getfirstentry ();
					getfilesize ();
					getfiletype (xferlist[firstentry].
						     fullname, MAXRECURSE);
				}
			} else {
				switch (download (s)) {
