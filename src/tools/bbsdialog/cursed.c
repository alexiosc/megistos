...
/*
 * $Id: cursed.c,v 2.0 2004/09/13 19:44:54 alexios Exp $
 *
 * $Log: cursed.c,v $
...
completeclass (char *s)
{
	...
	for (i = 0; i < cls_count; i++)
		if (sameto (s, cls_classes[i].name)) {
...
showstringfield (union object *obj)
{
	...
	for (i = 0; i < obj->s.shown; i++) {
		if ((!obj->s.data[i + startpos]) && !space)
			space = 1;
		addch (space ? 32
		       : ((obj->s.flags & 2) ? '*' : obj->s.
			  data[i + startpos] & 0xff));
	}
...
shownumfield (union object *obj)
{
	...
	sprintf (meta, " %%%ds ",
		 obj->n.flags & 1 ? obj->n.shown : -obj->n.shown);
...
showlistfield (union object *obj)
{
	...
	for (i = 0; obj->l.data[i]; i++)
		addch (obj->l.data[i] & 0xff);
	for (; i < obj->l.size; i++)
		addch (32);
}
...
showbuttonfield (union object *obj)
{
	...
	for (i = 0; i < j; i++)
		addch (32);
	for (i = 0; obj->b.label[i]; i++)
		addch (obj->b.label[i] & 0xff);
	for (i = 0; i < (obj->b.size - strlen (obj->b.label) - j); i++)
		addch (32);
	addch (obj->b.flags & 1 ? '<' : 32);
}
...
drawfield (int n)
{
	...
	case OBJ_NUM:
		...
		move (object[n].n.y, object[n].n.x +
		      (object[n].n.flags & 1 ?
		       object[n].n.shown + 1 + object[n].n.cp -
		       strlen (object[n].n.data) : object[n].n.cp + 1));
		break;
	case OBJ_LIST:
...
handlestringfield (int n, chtype c)
{
	...
	case 127:
		if ((i = strlen (obj->s.data)) > 0 && obj->s.cp) {
			...
			for (j = obj->s.cp - 1; j < i; j++)
				obj->s.data[j] = obj->s.data[j + 1];
			obj->s.data[i] = 0;
			obj->s.cp--;
			drawfield (n);
			refresh ();
	...
	default:
		if ((i = strlen (obj->s.data)) < obj->s.max && ISPRINT (c)) {
			...
			if ((obj->s.flags & 4) && (!isdigit (c)))
				c = '/';
			if (obj->s.flags & 1)
				c = toupper (c);
			for (j = i; j >= obj->s.cp; j--)
				obj->s.data[j + 1] = obj->s.data[j];
...
handlenumfield (int n, chtype c)
{
	...
	case 8:
		if ((i = strlen (obj->n.data)) > 0 && obj->n.cp) {
			...
			for (j = obj->n.cp - 1; j < i; j++)
				obj->n.data[j] = obj->n.data[j + 1];
			obj->n.data[i] = 0;
			obj->n.cp--;
			drawfield (n);
			refresh ();
	...
	case 4:
		...
		break;
	default:
		if ((i = strlen (obj->n.data)) < obj->n.shown &&
		    (isdigit (c) || c == '-')) {
			int     j;

			for (j = i; j >= obj->n.cp; j--)
				obj->n.data[j + 1] = obj->n.data[j];
...
handlelistfield (int n, chtype c)
{
	...
	case '_':
		if (obj->l.data != obj->l.options) {
			...
			for (cp = obj->l.options; *cp != 0 || *(cp + 1) != 0;
			     cp++);
			for (cp--; *cp; cp--);
			obj->l.data = cp + 1;
		}
