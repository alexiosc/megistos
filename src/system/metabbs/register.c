...
/*
 * $Id: register.c,v 2.0 2004/09/13 19:44:54 alexios Exp $
 *
 * $Log: register.c,v $
...
find_system (char *codename)
{
	...
	for (i = 0; i < num_systems; i++) {
		if (!strcmp (codename, registered_systems[i].codename))
			return i;
	}
	return -1;		/* Not found */
}
...
my_hostname ()
{
	...
	fprintf (stderr, "aliases:  ");
	{
		...
		for (i = 0; me->h_aliases[i]; i++)
			fprintf (stderr, "%s ", me->h_aliases[i]);
	}
	fprintf (stderr, "\n");
	...
	if (!strchr (me->h_name, '.')) {
		...
		hostname =
		    strdup (inet_ntoa
			    (*((struct in_addr *) me->h_addr_list[0])));
		return hostname;
	}
...
validate (registration_package_t * reg)
{
	...
	if (!strlen (reg->codename))
		return 0;
	if (!strlen (reg->bbstitle))
		return 0;
	if (reg->users_online < 0)
		return 0;
	if (reg->lines_free < 0)
		return 0;
	if (reg->lines_max < 0)
	...
	if ((reg->bbs_uid == 0) || (reg->bbs_gid == 0))
		return 0;
...
make_space ()
{
	...
	if (!num_systems) {
...
#ifdef DEBUG
		fprintf (stderr,
			 "This is our first registration for this session.\n");
#endif
...
#ifdef DEBUG
		fprintf (stderr,
			 "New (but not first) registration, making space.\n");
#endif
		...
		tmp = (struct registration_package_t *)
		    malloc (num_systems *
			    sizeof (struct registration_package_t));

		memcpy (tmp,
			registered_systems,
			sizeof (struct registration_package_t) * (num_systems -
								  1));
...
int    *
metabbs_register_1_svc (registration_package_t * reg, struct svc_req *client)
{
	static int retval = 0;
	int     i;
