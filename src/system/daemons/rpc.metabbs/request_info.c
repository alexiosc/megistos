...
/*
 * $Id: request_info.c,v 2.0 2004/09/13 19:44:53 alexios Exp $
 *
 * $Log: request_info.c,v $
...
#ifndef RCS_VER 
#define RCS_VER "$Id: request_info.c,v 2.0 2004/09/13 19:44:53 alexios Exp $"
...
int try_match(struct hostent *caller, char *tok)
{
  int i;
  ...
  if(sameas(caller->h_name,tok))return 1;
  ...
  for(i=0;caller->h_aliases[i];i++){
    ...
    if(sameas(caller->h_aliases[i],tok))return 1;
  }
...

  {
    ...
    if(inet_aton(tok,&tokaddr)) for(i=0;caller->h_addr_list[i];i++){
      
#ifdef DEBUG
      char *a=inet_ntoa(*((struct in_addr*)caller->h_addr_list[i]));
      fprintf(stderr,"ACL: comparing \"%s\" , \"%s\"\n",a,tok);
#endif
      
      if(!memcmp(&tokaddr,caller->h_addr_list[i],sizeof(tokaddr)))return 1;
    }
  }
...
int match_acl(char *acl, struct sockaddr_in *client, char *codename)
{
  ...
  {
    int i;
    for(i=0;caller->h_aliases[i];i++)fprintf(stderr,"%s ",caller->h_aliases[i]);
  }
  fprintf(stderr,"\n");
  ...
  fprintf(stderr,"addrs:    ");
  {
    ...
    for(i=0;caller->h_addr_list[i];i++)
      fprintf(stderr,"%s ",inet_ntoa(*((struct in_addr*)caller->h_addr_list[i])));
  }
  fprintf(stderr,"\n\nACL=\"%s\"\n",tmp);
#endif


  if((cp=strtok(tmp," \t\n\r"))==NULL) return -1; /* Empty */
  do {
    ...
    if((cn=strchr(tmp,'/'))!=NULL){
      if(!sameas(++cn,codename))continue;
      else return 1;
    }
    ...
    if(try_match(caller,cp))return 1;
...
{
  info_package_p retval=(info_package_p)malloc(sizeof(struct info_package_t));
...
metabbs_request_info_1_svc (char **codename, struct svc_req *req)
{
  ...
  if(retstuff!=NULL)xdr_free((xdrproc_t)xdr_info_package_p,(caddr_t)&retstuff);
  /*deallocate(retstuff);  Booh, can't use that cure function now */
  retstuff=NULL;


#ifdef DEBUG
    fprintf(stderr,"Processing request for information from BBS \"%s\"\n",
	    *codename);
    fprintf(stderr,"BBSs running here: %d\n",num_systems);
#endif

  for(i=0;i<num_systems;i++){
    register struct registration_package_t *bbs=&registered_systems[i];
    ...
    if((t-bbs->regtime)>SYSTEM_EXPIRY) continue;
...
#endif
    if(match_acl(bbs->access_allow,caller,*codename)==0) continue;
...
#endif
    if(match_acl(bbs->access_deny,caller,*codename)==1) continue;


    /* Is it full? Busy sites aren't reported. */

    if(bbs->lines_free==0) continue;


    /* Has it disabled MetaBBS access? */
    
    if(bbs->port==0) continue;
