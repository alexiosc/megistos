...
/*
 * $Id: distclub.c,v 2.0 2004/09/13 19:44:53 alexios Exp $
 *
 * $Log: distclub.c,v $
...
#ifndef RCS_VER 
#define RCS_VER "$Id: distclub.c,v 2.0 2004/09/13 19:44:53 alexios Exp $"
...
#ifdef DEBUG
    fprintf(stderr,"apply_prefix: This_system is null, returning fname\n");
#endif
    return fname;
  }
...
getclubaccess(struct sockaddr_in *caller, char *codename)
{
  ...
  if(!strcmp(tmp,"*")){	/* Club is open to all systems */
    ...
    for(cp=strtok(tmp," ");cp;cp=strtok(NULL," ")){
      if(!strcmp(cp,"-"))if(in_accept){
	in_accept=0;
	
	/* We just finished with the ACCEPT part -- make sure we match it */
	if(j>0) {
	  has_access=has_access || (match_acl(tmp2,caller,codename)==1);
	} else {
    ...
    /* Nope, must be in the DENY bit */
      ...
      has_access=has_access && (match_acl(tmp2,caller,codename)==0);
    }
  }
  return has_access;
}
...
findclub(char *club)
{
  ...
  if((dp=opendir(apply_prefix(CLUBHDRDIR)))==NULL) return 0;
...
club_list *
distclub_request_list_1_svc (struct club_list_request_t *bbs, struct svc_req *req)
{
  ...
  if(need_deallocation)xdr_free((xdrproc_t)xdr_club_list,(caddr_t)&retstuff);
  need_deallocation=0;
  ...
  for(i=0;i<n;free(clubs[i]),i++){
    ...
    if(getclubaccess(caller,bbs->codename)){
      ...
      fprintf(stderr,"OK: %-16.16s %-10.10s %-40.40s %s\n",
	      ...
	      clubhdr.descr,
	      clubhdr.export_access_list);
#endif
      
      if(clubitem==NULL){
	clubitem=retstuff.club_list_u.clubs=
	  (club_list_item_p)malloc(sizeof(club_list_item_t));
      } else {
	clubitem->next=(club_list_item_p)malloc(sizeof(club_list_item_t));
			      ...
			      struct svc_req *req)
{
...
#ifdef DEBUG
  fprintf(stderr,"Club \"%s\" header request from BBS \"%s\" for BBS \"%s\"\n",
  ...
  if(stat(fname,&st)){
    ...
    if((fp=fopen(fname,"r"))==NULL){
      ...
      retstuff.club_header_u.club.banner=(char*)malloc(st.st_size+1);
      retstuff.club_header_u.club.banner[0]=0;
      fread(retstuff.club_header_u.club.banner,st.st_size,1,fp);
