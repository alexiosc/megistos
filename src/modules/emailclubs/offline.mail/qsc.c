...
/*
 * $Id: qsc.c,v 2.0 2004/09/13 19:44:51 alexios Exp $
 *
 * $Log: qsc.c,v $
...
usaveqsc (char *uid)
{
	...
	if (fwrite (uqsc, sizeof (struct lastread), numclubs, fp) != numclubs) {
		error_fatalsys ("Unable to write quickscan configuration %s",
				fname);
	}
...
resetall (int n)
{
	...
	if (n < 0)
		emlu.lastemailqwk = max (n + sysvar->emessages, 0);
	else
		emlu.lastemailqwk = min (sysvar->emessages, n);
	...
	for (i = 0; i < j; free (clubs[i]), i++) {
		...
		if (p && (p->flags & LRF_INQWK)) {
			if (n >= 0)
				p->qwklast = min (clubhdr.msgno, n);
			else
				p->qwklast = max (0, clubhdr.msgno + n);
		}
	}
	free (clubs);
...
addclub ()
{
	...
	if (!getclub (club, OMCADD, OMCCR, 1, 0))
		return;
	if (sameas (club, "ALL"))
		all (ADD);
	else {
		...
		p = findqsc (club);
		if (!p)
			addqsc (club, 0, LRF_INQWK);
		else if (p->flags & LRF_INQWK)
			addqsc (club, clubhdr.msgno, p->flags);
		else
			addqsc (club, 0, p->flags | LRF_INQWK);
	}
}
...
delclub ()
{
	...
	for (;;) {
		if (!getclub (club, OMCDEL, OMCDR, 1, 0))
			return;
		if (sameas (club, "ALL")) {
			all (DEL);
			return;
...
resetclub ()
{
	...
	for (;;) {
		if (!getclub (club, OMCRES, OMCRR, 1, 1))
			return;
		if (sameas (club, "ALL")) {
			all = 1;
			break;
	...
	if (!get_number (&n, OMCRN,
			 email ? -sysvar->emessages : (all ? -99999 : -clubhdr.
						       msgno),
			 email ? sysvar->emessages : (all ? 99999 : clubhdr.
						      msgno), OMCRNR, 0, 0)) {
		return;
	...
	if (all)
		...
		if (!readecuser (thisuseracc.userid, &emlu)) {
			error_fatal
			    ("Unable to read E-mail preference file for %s",
			     thisuseracc.userid);
		}

		if (n < 0)
			n = max (n + sysvar->emessages, 0);
		emlu.lastemailqwk = n;

		if (!writeecuser (thisuseracc.userid, &emlu)) {
			error_fatal
			    ("Unable to write E-mail preference file for %s",
			     thisuseracc.userid);
		}
	} else {
		if (n < 0)
			n = max (n + clubhdr.msgno, 0);
		setlastread (club, n);
	}
}
...
configurequickscan (int create)
{
	...
	for (;;) {
		...
		i = get_menu (&opt, !shown, OMCMNU, OMCMNUS, OMERR, "+-VRFT",
			      0, 0);
		shown = 1;
