...
/*
 * $Id: clubcleanup.c,v 2.0 2004/09/13 19:44:50 alexios Exp $
 *
 * $Log: clubcleanup.c,v $
...
fwperiodic (message_t *msg)
{
	...
	if ((fp1 = fopen (header, "r")) == NULL) {
		printf
		    ("\n\nclubcleanup(): ERROR, Unable to open message header %s",
		     header);
...
clubcleanup ()
{
	...
	if ((dp = opendir (fname)) == NULL) {
		printf
		    ("clubcleanup(): can't open directory %s, cleanup not done\n",
		     fname);
		return;
	}
	...
	printf
	    ("Club name        MSGDEL KBDEL NMSGS NFILE UNAPP  NPER FWPER\n");
	...
	while ((dir = readdir (dp)) != NULL) {
		...
		msgdel = msgdelb = msglive = msgper = msgfile = msgunapp =
		    msgfper = 0;
		msgpop = msgdnl = 0;

		for (i = 0; i < n; i++) {
			dir = msgs[i];
			sprintf (fname, "%s/%s/%s", mkfname (MSGSDIR),
				 club.club, dir->d_name);
			...
			if (strncmp
			    (msg.club, club.club, strlen (club.club) + 1)) {
				fprintf (stderr,
					 "Fixing corrupted message: (%s)\n",
					 fname);
			...
			if ((club.msglife > 0) &&
			    ((msg.flags & MSF_EXEMPT) == 0) &&
			    (msg.period == 0)) {
				if ((ctoday - cofdate (msg.crdate)) >
				    club.msglife) {
					int     dbytes =
					    cleanup_erasemsg (&msg);
					if (dbytes != -1) {
						msgdel++;
						msgdelb += dbytes;
					}
		...
		if (((int32) club.msgno == -1) || (club.msgno < maxmsgno))
			club.msgno = maxmsgno;
		...
		printf ("%5d %5d %5d %5d %5d %5d %5d\n",
			msgdel, msgdelb >> 10, msglive, msgfile, msgunapp,
			msgper, msgfper);
	...
	for (pass = 0; pass < 2; pass++) {
		...
		if (!pass)
			...
			sprintf (dir, "%s/%ld", mkfname (STATDIR),
				 tdyear (today ()));
