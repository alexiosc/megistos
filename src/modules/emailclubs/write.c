...
/*
 * $Id: write.c,v 2.0 2004/09/13 19:44:50 alexios Exp $
 *
 * $Log: write.c,v $
 ...
 * crash.
 *
 * Revision 1.5  2003/12/25 13:33:27  alexios
...
emailwrite ()
{
	...
	FILE   *fp;
	int     attachment = 0, rrr = 0, numcopies = 0, nomore = 0, net =
	    0, res = 0;
	int     cleanupattachment = 1, distlist = 0, ndist = 0, killit =
	    0, sysop = 0;
	int     clubmsg = 0;
	uint32  original = 0;
	char    attfile[256] = { 0 }, origclub[256] = {
	0};
	...
	for (; !nomore;) {
		...
		if (!distlist) {	/* distribution list still running */
			if (sysop) {
			    ...
			    if ((res =
				 getrecipient (numcopies ? WEWHOC : WEWHO,
					       msg.to)) == 0)
				return;
			...
			if (msg.to[0] != '!') {
				net = strchr (msg.to, '@') ||
				    strchr (msg.to, '%');
				if (!clubmsg)
					prompt (net ? WERCPOKN : WERCPOK,
						msg.to);
				else if (sameas (msg.to, MSG_ALL))
					prompt (WERCPOKC, msg.club);
				else
					prompt (WERCPOKU, msg.club, msg.to);
			} else {
				prompt (WERCPOKL, msg.to);	/* open distributinon list */
				distlist = 1;
				ndist = 0;
				if (!opendistribution (msg.to))
					return;
			}
		}
		...
		if (!numcopies) {
			...
			sprintf (body, TMPDIR "/B%d%08lx", getpid (),
				 time (0));
			appendsignature (body);
			sprintf (header, TMPDIR "/H%d%08lx", getpid (),
				 time (0));
			...
			if (((clubmsg == 0) && key_owns (&thisuseracc, attkey)
			     && usr_canpay (attchg)) || (clubmsg &&
							 (getclubax
							  (&thisuseracc,
							   msg.club) >=
							  CAX_UPLOAD) &&
			...
			if (key_owns (&thisuseracc, rrrkey) &&
			    usr_canpay (rrrchg) && (clubmsg == 0)) {
				for (;;) {
					if (!askyesno
					    (&rrr, WERRR, WERRSEL, rrrchg)) {
						if (confirmcancel ()) {
							unlink (body);
							unlink (header);
							return;
			...
			if (rrr)
				msg.flags |= MSF_RECEIPT;
			if (!distlist)
				sprintf (msg.history, HST_CC " %s/%d",
					 origclub, original);
			else
		...
		if (msg.to[0]) {	/* handle end of distribution list */
			if ((fp = fopen (header, "w")) == NULL) {
				error_fatalsys
				    ("Unable to create message header %s",
				     header);
			}
			if (fwrite (&msg, sizeof (msg), 1, fp) != 1) {
				...
				errno = i;
				error_fatalsys
				    ("Unable to write message header %s",
				     header);
			}
			fclose (fp);
			...
			if ((thisuseronl.flags & OLF_INVISIBLE) == 0) {
				if ((fp = fopen (header, "r")) == NULL) {
					error_fatalsys
					    ("Unable to read message header %s",
					     header);
				}
				if (fread (&checkmsg, sizeof (msg), 1, fp) !=
				    1) {
					int     i = errno;
			...
			if (attfile[0] && cleanupattachment) {
				unlink (attfile);
				strcpy (attfile,
					mkfname ("%s/%s/" MSGATTDIR "/"
						 FILEATTACHMENT, MSGSDIR,
						 clubmsg ? clubhdr.
						 club : EMAILDIRNAME,
						 original));
			}
...
recipienthelp ()
{
	...
	if ((dp = opendir (mkfname (MSGSDISTDIR))) == NULL)
		return;
	while ((dir = readdir (dp)) != NULL) {
		if (sameas (dir->d_name, ".") || sameas (dir->d_name, ".."))
			continue;
		if (fmt_lastresult == PAUSE_QUIT) {
			closedir (dp);
			return;
		...
		if (key_owns (&thisuseracc, key) &&
		    (dir->d_name[0] != '.' ||
		     sameas (&(dir->d_name[1]), thisuseracc.userid) ||
...
checklistname (char *name)
{
	...
	if (key_owns (&thisuseracc, msskey)) {
		if (name[0] == name[1]) {
			for (i = 0; i < cls_count; i++)
				if (sameas (cls_classes[i].name, &name[2])) {
					return 1;
				}
			prompt (EWRLERR);
			return 0;
	...
	if (!strcmp (name, "!"))
		sprintf (name, "!.%s", thisuseracc.userid);
...
getrecipient (int pr, char *rec)
{
	...
	for (;;) {
		...
		if (sameas (s, ".")) {
			...
			if (!findclub (s)) {
				prompt (WCCLUBR);
			} else if ((ax = getclubax (&thisuseracc, s)) ==
				   CAX_ZERO) {
				prompt (WCCLUBR);
			...
			strcpy (rec, s);
			if (usr_canpay (wrtchg) || sameas (rec, SYSOP))
				return 1;
			else
				continue;
...
getsubject (char *subject)
{
	...
	for (;;) {
		...
		if (!i[0])
			...
			strcpy (subject, i);
			while (subject[strlen (subject) - 1] == 32)
...
uploadatt (char *attname, int num)
{
	...
	sprintf (audit[1], AUD_ESUPLS, thisuseracc.userid, clubdir, num);
	strcpy (audit[2], AUS_ESUPLF);
	sprintf (audit[3], AUD_ESUPLF, thisuseracc.userid, clubdir, num);
	xfer_setaudit (AUT_ESUPLS, audit[0], audit[1], AUT_ESUPLF, audit[2],
		       audit[3]);
...
getattname (char *subject, int num)
{
	...
	cp = strchr (word1, '.');
	if (cp)
		ok = (*(cp + 1) != 0 && !(isspace (*(cp + 1))) &&
		      isprint (*(cp + 1)));
	for (i = 0; word1[i]; i++)
		if (!strchr (FNAMECHARS, word1[i]))
			ok = 0;
	if (ok)
		return (latinize (word1));
	else
		sprintf (apparent, "%d.att", num);
