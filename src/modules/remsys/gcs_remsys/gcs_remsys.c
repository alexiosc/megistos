...
/*
 * $Id: gcs_remsys.c,v 2.0 2004/09/13 19:44:52 alexios Exp $
 *
 * $Log: gcs_remsys.c,v $
...
gcs_invis ()
{
	if (margc == 1 && sameas (margv[0], "/invis")) {
		if (hassysaxs (&thisuseracc, USY_INVIS)) {
			...
			msg_set (msg_sys);
			prompt ((thisuseronl.
				 flags & OLF_INVISIBLE) ? INVON : INVOFF);
			msg_reset ();
			return 1;
		}
	}
...
gcs_gdet ()
{
	...
	if (margc == 2 && sameas (margv[0], "/l") &&
	    hassysaxs (&thisuseracc, USY_GDET)) {
		...
		strcpy (d2,
			(uacc->datelast >= 0) ? strdate (uacc->datelast) : "");
		prompt (GDET, userid, d1, s1, s2, d2, s3, s4,
			(key_owns (&thisuseracc, kgdnam) ? uacc->
			 username : wh), (key_owns (&thisuseracc,
						    kgdcom) ? uacc->
					  company : wh),
			(key_owns (&thisuseracc, kgdadr) ? uacc->
			 address1 : wh), (key_owns (&thisuseracc,
						    kgdadr) ? uacc->
					  address2 : wh),
			(key_owns (&thisuseracc, kgdpho) ? uacc->phone : wh),
			(key_owns (&thisuseracc, kgdage) ? age : wh),
			(key_owns (&thisuseracc, kgdsex) ? sex : wh), sys,
			uacc->scnwidth, uacc->scnheight, ns, lang,
			uacc->curclss, ((!kgdpass) &&
					key_owns (&thisuseracc,
						  kgdpss) ? uacc->passwd : wh),
...
gcs_remsys ()
{
	if (margc == 1 &&
		...
		for (i = j = 0;
		     i < sizeof (thisuseracc.sysaxs) / sizeof (long); i++)
...
gcs_audit ()
{
	if (!hassysaxs (&thisuseracc, USY_PAGEAUDIT))
		return 0;
	if (margc && sameas (margv[0], "/audit")) {
		if (margc == 1) {
			...
			if (sameas ("o", margv[1])) {
				...
				thisuseracc.auditfiltering &= ~AUF_SEVERITY;
				thisuseracc.auditfiltering |=
				    AUF_EMERGENCY | AUF_CAUTION;
				prompt (AUDITC);
