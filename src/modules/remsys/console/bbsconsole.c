...
/*
 * $Id: bbsconsole.c,v 2.0 2004/09/13 19:44:52 alexios Exp $
 *
 * $Log: bbsconsole.c,v $
 ...
 * crash.
 *
 * Revision 1.5  2004/02/29 16:55:05  alexios
 * One minor #include fix.
 *
 * Revision 1.4  2003/12/31 08:27:21  alexios
...
waitchannels ()
{
	...
	for (j = 0; j < 30; j++) {
		...
		for (i = 0; i < chan_count; i++) {
			sprintf (fname, "%s/.status-%s", mkfname (CHANDEFDIR),
				 channels[i].ttyname);
			bzero (&st, sizeof (st));
			stat (fname, &st);
			if (st.st_mtime > t) {
