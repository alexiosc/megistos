...
/*
 * $Id: account.c,v 2.1 2004/09/13 19:55:34 alexios Exp $
 *
 * $Log: account.c,v $
...
editaccount ()
{
	...
	if (sameas (margv[10], "OK") || sameas (margv[10], margv[8])) {
		...
		for (j = 0; j < 8; j++)
			if (!strcmp (margv[7], msg_get (SYS1 + j))) {
				tempacc.system = j + 1;
				break;
			}
		...
		if (lognam && strcmp (tempacc.username, uacc->username))
			audit (thisuseronl.channel, AUDIT (ACCEDT1),
			       uacc->userid, "NAME");
		if (logcmp && strcmp (tempacc.company, uacc->company))
			audit (thisuseronl.channel, AUDIT (ACCEDT1),
			       uacc->userid, "COMPANY");
		if (logadr &&
		    (strcmp (tempacc.address1, uacc->address1) ||
		     strcmp (tempacc.address2, uacc->address2)))
			audit (thisuseronl.channel, AUDIT (ACCEDT1),
			       uacc->userid, "ADDRESS");
		if (logphn && strcmp (tempacc.phone, uacc->phone))
			audit (thisuseronl.channel, AUDIT (ACCEDT3),
			       uacc->userid, "PHONE");
		if (logage && tempacc.age != uacc->age)
			audit (thisuseronl.channel, AUDIT (ACCEDT2),
			       uacc->userid, "AGE", uacc->age, tempacc.age);
		if (logsex && tempacc.sex != uacc->sex)
			audit (thisuseronl.channel, AUDIT (ACCEDT3),
			       uacc->userid, "SEX");
		if (logsys && tempacc.system != uacc->system)
			audit (thisuseronl.channel, AUDIT (ACCEDT2),
			       uacc->userid, "SYSTEM", uacc->system,
			       tempacc.system);
...
changepassword (char *userid, int login)
{
			...
			if (!sameas (inp_buffer, uacc->passwd)) {
				...
				if (!login)
					...
					audit (thisuseronl.channel,
					       AUDIT (BLGPWCH),
					       thisuseracc.userid,
					       channel_baudstg (thisuseronl.
								baudrate));
					channel_disconnect (thisuseronl.
							    emupty);
				}
			} else
				break;
	...
	for (;;) {
		if (mkpswd && mkpwdf) {
			...
			if (mkpswd)
				prompt (RECPSW, recommended, NULL);
			inp_setflags (INF_PASSWD);
			prompt (GPSWORD, NULL);
			inp_get (15);
			inp_clearflags (INF_PASSWD);
...
resetbbslib ()
{
	...
	if ((thisuseracc.prefs & (UPF_ANSIDEF | UPF_ANSIASK)) == 0) {
		SETVALUE (thisuseronl.flags, OLF_ANSION,
			  thisuseracc.prefs & UPF_ANSION);
		if (thisuseronl.flags & OLF_ANSION)
			out_setflags (1);
		else
			out_clearflags (0);
	}
	fmt_setverticalformat ((thisuseracc.prefs & UPF_NONSTOP) == 0);
	if ((thisuseracc.prefs & UPF_NONSTOP) == 0)
...
editprefs ()
{
	...
	inp_buffer[0] = '\0';
	if (tempacc.prefs & UPF_ANSIDEF)
		sprintf (tmp, "%s\n", msg_get (ANSDEF));
	else if (tempacc.prefs & UPF_ANSIASK)
		sprintf (tmp, "%s\n", msg_get (ANSASK));
	else if (tempacc.prefs & UPF_ANSION)
		sprintf (tmp, "%s\n", msg_get (ANSON));
	else
		sprintf (tmp, "%s\n", msg_get (ANSOFF));
	strcat (inp_buffer, tmp);

	if (tempacc.prefs & UPF_TRDEF)
		sprintf (tmp, "%s\n", msg_get (TRDEF));
	else if (tempacc.prefs & UPF_TRASK)
		sprintf (tmp, "%s\n", msg_get (TRASK));
	else
		sprintf (tmp, "%s\n",
	...
	sprintf (tmp, "%s\n%s\n%s\n%d\n%d\n",
		 ...
		 tempacc.scnwidth, tempacc.scnheight);
	strcat (inp_buffer, tmp);

	sprintf (tmp, "%s\n%d\nOK\nCancel\n",
		 msg_get (PGSSTR + tempacc.pagestate), tempacc.pagetime);
	strcat (inp_buffer, tmp);
	...
	if (sameas (margv[11], "OK") || sameas (margv[11], margv[9])) {
		for (i = 0; i < 12; i++) {
			...
			if (i == 0) {
				SETVALUE (tempacc.prefs, UPF_ANSIDEF,
					  sameas (s, msg_get (ANSDEF)));
				SETVALUE (tempacc.prefs, UPF_ANSIASK,
					  sameas (s, msg_get (ANSASK)));
				SETVALUE (tempacc.prefs, UPF_ANSION,
					  sameas (s, msg_get (ANSON)));
			} else if (i == 1) {
				int     n;

				SETVALUE (tempacc.prefs, UPF_TRDEF,
					  sameas (s, msg_get (TRDEF)));
				SETVALUE (tempacc.prefs, UPF_TRASK,
					  sameas (s, msg_get (TRASK)));
				...
				SETVALUE (tempacc.prefs, UPF_NONSTOP,
					  sameas (s, "on"));
			else if (i == 3)
				SETVALUE (tempacc.prefs, UPF_VISUAL,
					  sameas (s, "on"));
			else if (i == 4) {
				int     j;

				for (j = 0; j < 10; j++)
					if (!strcmp (s, msg_get (LANG1 + j))) {
						tempacc.language = j + 1;
						break;
					}
			} else if (i == 5)
				tempacc.scnwidth = atoi (s);
			else if (i == 6)
				tempacc.scnheight = atoi (s);
			else if (i == 7) {
				int     j, k[4] =
				    { PGS_STORE, PGS_OK, PGS_ON, PGS_OFF };
				for (j = 0; j < 4; j++)
					if (!strcmp (s, msg_get (PGSSTR + j))) {
						tempacc.pagestate = k[j];
						break;
					}
			} else if (i == 8)
				tempacc.pagetime = atoi (s);
		}

		memcpy (uacc, &tempacc, sizeof (tempacc));
		if (sameas (uacc->userid, thisuseracc.userid))
			resetbbslib ();
		prompt (EDITOK);
	} else {
		prompt (EDITCAN);
...
transfercredits ()
{
	...
	if (!get_number
	    (&num, CREDTR, mincxf, min (uacc->credits - 1, maxcxf), NUMERR, 0,
	     0)) {
		prompt (CXFABO);
		return;
	}
	...
	if (!usr_exists (userid)) {
		...
		audit (thisuseronl.channel, AUDIT (CRDXFER), uacc->userid, num,
		       userid);
		prompt (CXFOK, num, uacc->userid, userid);
		if (usr_insys (userid, 0)) {
			char    injbuf[16384];

			sprompt_other (othrshm, injbuf, CXFINJ, uacc->userid,
				       num);
			if (usr_injoth (&othruseronl, injbuf, 0))
				prompt (NOTICE, ruacc->userid);
		}
	}
}
...
showstats ()
{
	...
	if (uacc->datecre != today ()) {
		...
		sprompt (out_buffer, ACCST2A,
			 msg_getunit (DAYSNG, uacc->passexp));
	...
	if (class->crdpermonth) {
		...
		sprompt (out_buffer, ACCST5D, class->crdpermonth,
			 msg_getunit (CRDSNG, class->crdpermonth), hoften);
		strcat (buf, out_buffer);
	}

	if (class->flags & CLF_NOCHRGE)
		strcat (buf, msg_get (ACCST5E));
	nod = class->ardays - uacc->classdays;
	if (class->ardays > 0) {
		sprompt (out_buffer, ACCST5F, nod, msg_getunit (DAYSNG, nod),
			 class->around);
		strcat (buf, out_buffer);
	...
	if (class->nadays != -1) {
		sprompt (out_buffer, ACCST5G, class->nadays,
			 msg_getunit (DAYSNG, class->nadays),
			 class->notaround);
		strcat (buf, out_buffer);
	}
...
passother ()
{
	...
	if (!get_userid (userid, OPSWHO, OUIDERR, 0, NULL, 0))
		return;
	else {
