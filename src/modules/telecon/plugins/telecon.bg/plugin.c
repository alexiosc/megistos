...
/*
 * $Id: plugin.c,v 2.0 2004/09/13 19:44:53 alexios Exp $
 *
 * $Log: plugin.c,v $
...
controllingpart ()
{
	...
	if (margc == 1) {
		if (sameas (margv[0], "-")) {
		    ...
		    if (!(sameas (margv[0], "?") || sameas (margv[0], "help")))
		{
			char   *userid = margv[0];
