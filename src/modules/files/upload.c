...
/*
 * $Id: upload.c,v 2.0 2004/09/13 19:44:51 alexios Exp $
 *
 * $Log: upload.c,v $
 ...
 * crash.
 *
 * Revision 1.5  2003/12/27 08:59:38  alexios
 * Adjusted #includes.
 *
...
{
	long int nb =
	    (library.libsizelimit << 20) - library.numbytes -
	    library.bytesunapp;
	...
	if (library.numbytes + library.bytesunapp >=
	    (library.libsizelimit << 20)) {
		prompt (UPLXLNB);
		return LIM_CANCEL_ALL;
	}
...
checkfname (char *s)
{
	...
	if (fileexists (library.libnum, s, 0) ||
	    fileexists (library.libnum, s, 1)) {
		prompt (UPLDUP);
		return 0;
	}
	...
	if (library.flags & LBF_DOS83) {
		...
		} else if ((cp - s < 1) || (cp - s > 8) || (strlen (cp) > 4) ||
			   (strchr (cp + 1, '.') != NULL)) {
			prompt (FDNDOS);
			return 0;
		}
...
filehdr (char *fname, struct fileidx *f, char *keywords)
{
	...
	prompt (FILEHDR1, f->fname,
		...
		f->uploader, f->downloaded, msg_getunit (TIMESNG,
							 f->downloaded),
		st.st_size);

	if (f->approved)
		prompt (FILEHDR2, f->approved_by);
...
fileheader (char *name, int multiple, char *source, int filenum, int numfiles)
{
	...
	if (multiple) {
		...
		if (!rem)
			strcpy (tmp, msg_get (REMZER));
		else
			sprintf (tmp, msg_get (rem == 1 ? REMSNG : REMPLR),
				 rem);
		np = strrchr (name, '/');
		if (np)
			np++;
...
confirmcan ()
{
	...
	if (!get_bool (&yes, UPLACN, UPLACR, 0, 0))
		return 0;
	if (yes)
		return 1;
	return 0;
	...
	for (n = 0; n < MAXARCHIVERS; n++) {
		...
		if (strlen (msg_get (ARCHIVERS_UNCOMPRESS1 + n * 7)) &&
			...
			sprintf (fmt, "cd %s;%s >&/dev/null;cd -",
				 dir, msg_get (ARCHIVERS_UNCOMPRESS1 + n * 7));
			...
			while (cp) {
				...
				if (!stat (command, &st)) {
					...
					if ((fp =
					     fopen (command, "r")) == NULL) {
						error_fatalsys
						    ("Sanity check failed: unable to open %s",
						     command);
					}
...
getkeywords ()
{
	...
	for (; numkeywords <= maxkeyw;) {
		...
		if (!strlen (inp_buffer)) {
			...
			break;
		} else if (strspn (inp_buffer, KEYWORDCHARS) !=
			   strlen (inp_buffer) || strlen (inp_buffer) > 23) {
			cnc_end ();
			prompt (UPLKEYR);
			continue;
		} else {
...
acceptfile (char *name, int multiple, char *origsrc, int filenum, int numfiles)
{
	...
	if ((longdescr = getlongdescr (name, ftype)) == NULL)
		return 0;
	...
	if (opt == 'E') {
		...
		if (library.flags & LBF_UPLAUD) {
			...
			audit (thisuseronl.channel, AUT_FUPLOK, AUS_FUPLOK,
			       tmp, thisuseracc.userid, f->fname);
		}
...
sizewarn ()
{
	...
	prompt (SIZEWARN, nf, files, nb, bytes1, bpf,
		msg_getunit (BYTESNG, bpf));
}
...
upload ()
{
	...
	if ((fn = getuploadfname (&multiple)) == NULL)
		return;
	tmp = strdup (fn);
