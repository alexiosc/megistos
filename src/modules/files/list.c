...
/*
 * $Id: list.c,v 2.0 2004/09/13 19:44:51 alexios Exp $
 *
 * $Log: list.c,v $
...
traverselibtree (struct libidx *l, char *br)
{
	...
	if (!getlibaccess (l, ACC_VISIBLE))
		...
		for (i = 0; br[i]; i++) {
			if (br[i + 1] == 0 && br[i] == '1')
				strcat (branches, msg_get (LITRBR));
			else
				strcat (branches,
					msg_get (LITRVR + br[i] - '1'));
			if (br[i] == '2')
				br[i] = '3';
		}
...
listsublibs ()
{
	...
	if (!libread (libmain, 0, &l)) {
		...
		prompt (LIBLSTL, "/", l.numfiles, l.numbytes >> 10, l.descr);
	...
	if (nesting (library.fullname)) {
		...
		prompt (LIBLSTP, l.numfiles, l.numbytes >> 10,
			leafname (l.fullname));
	}
...
normallisting (int approved)
{
	...
	if (i)
		...
		prompt (FLFTR, numfiles, numbytes);
	inp_block ();
