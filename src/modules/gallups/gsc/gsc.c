...
/*
 * $Id: gsc.c,v 2.0 2004/09/13 19:44:52 alexios Exp $
 *
 * $Log: gsc.c,v $
...
main (int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++) {
		if (argv[i][0] == '-') {
			...
			case 'e':
				switch (argv[i][2]) {
				case 't':
					gtset (ginfo) = atoi (&argv[i][3]);
					break;
				case 'd':
					gdset (ginfo) = atoi (&argv[i][3]);
					break;
				}
				break;
	...
	if (gallupname[0])
		...
		if (strrchr (filename, '/'))
			strcpy (gallupname, strrchr (filename, '/') + 1);
		else
			strcpy (gallupname, filename);
	...
	if (scriptcompiler (filename)) {
		...
		if (gscflags & INFO) {
			...
			show ("User ID's will %s logged\n",
			      (gflgs (ginfo) & GF_LOGUSERID) ? "be" :
			      "not be");
			show ("Users can submit gallup %s once\n",
			      (gflgs (ginfo) & GF_MULTISUBMIT) ? "more than" :
			      "only");
			show ("The results are available to %s\n",
			      (gflgs (ginfo) & GF_VIEWRESALL) ? "all users" :
			      "sysops only");
