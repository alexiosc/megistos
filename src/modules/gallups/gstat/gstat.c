...
void
parse_commandline (int vargc, char *vargv[])
{
	int     i, j;
			...
			case 'g':
				for (j = 2; j < strlen (vargv[i]); j++)
					...
					case '3':
						...
						break;
					case '4':
						graphmode |= FL_OPTSEXAGE;
						break;
					}
				break;
			case 'l':
				userlang = vargv[i][2] - '0';
				if (userlang == 0)
					userlang = 1;
				break;
			}
...
analyze_question (int qi)
{
	...
	if (qtyp (curquest) != GQ_SELECT && qtyp (curquest) != GQ_COMBO)
		return;

	sprintf (filename, "%s/%s/%s%i", GALLUPSDIR, gfnam (ginfo), GRESFILE,
		 qi);
	fp = fopen (filename, "r");
