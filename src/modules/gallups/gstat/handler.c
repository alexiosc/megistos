...
int
handler_register (handler_t handler)
{
	if (handler_count >= GMAXCLASS)
		return 0;
...
void
class_options_handler (int cmd, void *d)
{
	switch (cmd) {
	case CMD_SIZE:
...
void
class_choices_handler (int cmd, void *d)
{
	switch (cmd) {
	case CMD_SIZE:
...
void
class_age_handler (int cmd, void *d)
{
	int     i;
	...
	case CMD_IDX:
		...
		fprintf (stderr,
			 "User age %i could not be positioned in any valid division\n",
			 user.age);
		exit (1);
		break;
	case CMD_PRM:
		{
			...
			if (!mn && !mx) {
				fprintf (stderr,
					 "Error in prompt for division %i\n",
					 *(int *) d);
				exit (1);
			}
...
void
class_sex_handler (int cmd, void *d)
{
	switch (cmd) {
	case CMD_SIZE:
		*(int *) d = 2;
...
int
class_sum (treekey_t key, treekey_t mask)
{
	int     i, sum = 0;
	struct treenode *n;
	treekey_t kk;
