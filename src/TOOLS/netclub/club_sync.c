...
static int
do_message(CLIENT *cl, char *host, char *codename, char *club, ihave_list_p p)
{
  struct netqueryc         q;
  ...
  if(result->result_code)return 0;
  ...
  sprintf(hdr.history,"%s %s/%s/%s/%d",HST_NET,host,codename,club,p->msgno);
  ...
  sprintf(fname_h,"/tmp/netclubh-%x%x",(int)getpid(),(int)time(NULL));
  ...
  sprintf(fname_b,"/tmp/netclubb-%x%x",(int)getpid(),(int)time(NULL));
  ...
  if(!netmsg->comp_result.compression){
    int b=write(1,netmsg->message.message_val,netmsg->message.message_len);
    ...
    tmp=alcmem(i);
    uncompress(tmp,&i,netmsg->message.message_val,netmsg->message.message_len);
    if(write(fd,tmp,i)!=i){
      int i=errno;
      fprintf(stderr,"FATAL: unable to write to %s: %s\n",fname_b,strerror(i));
      exit(1);
    ...
    free(tmp);
#else
    fprintf(stderr,"FATAL: we didn't ask for compressed messages!\n");
    exit(1);
#endif
  }
  ...
  if(hdr.flags&MSF_FILEATT){
    sprintf(fname_a,"/tmp/netcluba-%x%x",(int)getpid(),(int)time(NULL));
    if((fd=open(fname_a,O_CREAT|O_WRONLY|O_TRUNC,0660))<0){
      int i=errno;
      fprintf(stderr,"Unable to create %s: %s\n",fname_a,strerror(i));
      return 1;
    }
    ...
    if(!netmsg->comp_result.compression){
      int b=write(1,netmsg->attachment.attachment_val,
...
do_club(char *sysname, char *rem, char *loc, int ihavedate)
{
  ...
  if(debug)fprintf(stderr,"  rem=(%s), loc=(%s), ihavedate=(%d)\n",
		   rem,loc,ihavedate);
  ...
  *cp2++=0;			/* cp2=remote club name */
  ...
  if(debug)fprintf(stderr,"Sending IHAVE list request for club %s/%s/%s\n",
		   tmp,cp,cp2);
  ...
  if(result->result_code)return ihavedate;

  p=result->ihave_list_u.ihave_list;
  while(p){
    if(debug)fprintf(stderr,
		     "IHAVE RECORD: ID=%s/%s/%s TIME=%d MSGNO=%d\n",
		     p->codename,p->orgclub,p->msgid,p->time,p->msgno);
...
club_sync(char *fname, char *sysname)
{
  ...
  if(force){
    ...
    printf("Syncing system %s: sync time elapsed, importing messages.\n",
	   sysname);
  }
  ...
  while(!feof(fp)){
	...
	final_ihavedate=do_club(sysname,rem,loc,ihavedate);
	if(first)mindate=final_ihavedate;
	else if(final_ihavedate<mindate)mindate=final_ihavedate;
      }
    }
  }
}
