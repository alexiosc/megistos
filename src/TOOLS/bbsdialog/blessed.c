  ...
  while ((c=*(msg_buffer++))!=0){
	...
	case 'b':
	  ...
	  if(sscanf(parms,"%d;\"%n",&obj.b.size,&i)==1){
	    ...
	    obj.b.key=*cp;
	    strncpy(obj.b.label,(char *)(cp+1),strlen(cp));

	    dataline[0]=0;
	    fgets(dataline,sizeof(dataline),data);

	    strcpy(obj.b.template,buffer);
	    memset(buffer,0,sizeof(buffer));
	    bp=buffer;
...
showstringfield(int i)
{
  ...
  if(object[i].s.shown < strlen(object[i].s.data)){
    ...
    if(object[i].s.flags&1)for(j=0;format[j];j++)format[j]=toupper(format[j]);
    if(object[i].s.flags&2)for(j=0;format[j];j++)format[j]='*';
    strcat(format,"...");
  } else {
...
shownumfield(int i)
{
  ...
  sprintf(meta,"%%%ds",
	  object[i].n.flags&1?object[i].n.shown:-object[i].n.shown);
...
getstringfield(int i)
{
  ...
  for(;;){
	...
	for(j=0;object[i].s.data[j];j++){
	  object[i].s.data[j]=toupper(object[i].s.data[j]);
	}
      }
...
getlistfield(int obj)
{
  ...
  for(;;){
	...
	for(i=1,cp=object[obj].l.options;*cp;cp+=strlen(cp)+1,i++)if(i==opt){
...
mainloop()
{
  ...
  for(;;){
	...
	if(i){
      ...
      for(i=0;i<numobjects;i++){
	if(j==i+1 && object[i].s.type!=OBJ_BUTTON){
	  found=1;
	  switch(object[i].s.type){
	  case OBJ_STRING:
	    getstringfield(i);
	...
	} else if(strlen(command)==1 && object[i].s.type==OBJ_BUTTON &&
