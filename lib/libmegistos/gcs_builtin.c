...
gcs_hash()
{
	...
	for(i = 0; i < chan_count; i++){
		...
		if (sscanf (line,"%s %s %d %s", majst, minst, &baud, user) != 4)
			continue;
			...
			if(!strcmp (user,"[SIGNUP]")){
				...
				if (usr_insystem (user, !ISYSOP, &ushm)){
					...
					prompt (WHOLINE2,
						...
						status,
						invis,
						ushm->onl.descr [(int) thisuseracc.language-1]);
				}
				shmdt((char *)ushm);
			}
		}
...
gcs_license()
{
	if (!margc) return 0;
		 ...
		 (sameas (margv [0], "/license") || sameas (margv [0], "/gpl"))) {
		char fname[256];
		...
		return 1;
		   ...
		   (sameas (margv [0], "/rules") || sameas (margv [0], "/conditions"))) {
		char fname[256];
	...
	if  (!(othruseronl.flags & OLF_INVISIBLE) || ISYSOP) {
		...
		if (!hassysaxs (&othruseracc, USY_MASTERKEY)
			...
			sprompt_other (othrshm, injbuf,
				       othruseronl.flags & OLF_BUSY? PAGMSG1: PAGMSG2,
			...
			inp_raw ();
			sprompt_other (othrshm, injbuf,
				       othruseronl.flags& OLF_BUSY? PAGNOT1: PAGNOT2,
				       msg_getunit (SEXM, thisuseracc.sex == USX_MALE),
	...
	if (margc && sameas (margv [0], "/p")) {
		...
		if (!key_owns (&thisuseracc, sysvar->pagekey)) {
			...
			if (sameas (margv [1], "on")) {
				...
				for(i = 0; i < chan_count; i++) {
					if (channel_getstatus (channels [i].ttyname,
							       &status)<0)
						continue;
				...
				for(i = 0; i < chan_count; i++) {
					...
					if (status.result == LSR_USER) {
						if(usr_insys (status.user, !ISYSOP)) {
							...
							if (othruseronl.flags&OLF_INVISIBLE
							    && (!ISYSOP)) continue;
