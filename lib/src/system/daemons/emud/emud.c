...
makeshm(char *tty)
{
  ...
  int shmid=0;
  struct shmid_ds buf;
  struct passwd *passwd=getpwnam(BBSUSERNAME);
  
  bbsuid=passwd->pw_uid;
  bbsgid=passwd->pw_gid;
  ...
  while (1) {
...
#ifdef DEBUG
      fprintf(stderr,"\n\r---(res=%d err=%d,%s) ",sel,tv.tv_sec,strerror(tv.tv_sec));
      if(FD_ISSET(STDIN_FILENO,&readset))fprintf(stderr,"[userinp] ");
      if(FD_ISSET(ptyfd,&readset))fprintf(stderr,"[output] ");
      if(FD_ISSET(emuin,&readset))fprintf(stderr,"[emuinp] ");
#endif
    } while (sel == -1);
    ...
    if(sel==0){
	 ...
	 time() system calls. */
      ...
      if(idlzapt) if(tmout+idlzapt < time(NULL)) gracefulexit();
      continue;
    }
    ...
    if (FD_ISSET(STDIN_FILENO, &readset)) {
      ...
      if(emuq->xlation>0){
	buff[n]=0;
	if(emuq->xlation>0)faststgxlate(buff,kbdxlation[emuq->xlation]);
      }
      write (ptyfd, buff, n);
      tmout=time(NULL);
    }
	...
	if(emuq->xlation>0){
	  buff[n]=0;
	  if(emuq->xlation>0)faststgxlate(buff,xlation[emuq->xlation]);
	}
	write (ptyfd, buff, n);
      }
    }
