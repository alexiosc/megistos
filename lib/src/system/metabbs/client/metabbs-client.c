 ...
 *
 * $Log$
 * Revision 1.1  2001/04/16 15:00:58  alexios
 * Initial revision
 *
 * Revision 0.1  2000/12/08 15:13:51  alexios
 * Initial checkin.
 *
 *
 */
...
int find_system(char *codename)
{
  ...
  for(i=0;i<num_systems;i++){
    if(!strcmp(codename,systems[i].codename))return i;
  }
  return -1;			/* Not found */
}
...
better(struct info_package_t *sys,struct info_package_t *existing)
{
  ...
  if((strlen(sys->hostname)==0) && (sys->port<0)) score_a+=20000000;
  if((strlen(existing->hostname)==0) && (existing->port<0)) score_b+=10000000;
...
{
  int i;
  if((i=find_system(sys->codename))<0) i=make_space();
  else {
    ...
    if(!better(sys,&systems[i]))return;
  }
...
int syscmp(const void *a, const void *b)
{
...
init()
{
  ...
  if(mode || showlocal){
    ...
    local.lines_free=max(0,sysvar->tnlmax-chan_telnetlinecount());
  ...
  if((cp=getenv("LANG"))!=NULL){
    if(atoi(cp)>0)msg_setlanguage(atoi(cp));
  }
...
gather()
{
  ...
  if(tmphosts==NULL)tmphosts=strdup(hosts); /* Allocate space */
  ...
  for(host=strtok(tmphosts," \n\r\t");host;host=strtok(NULL," \n\r\t")){
    if(!mode)prompt(LOOKUP,host);


    /* Create the RPM client for this host */

    if((cl=clnt_create(host,METABBS_PROG,METABBS_VERS,"tcp"))==NULL){
  ...
  if(!num_systems){		/* No systems found despite our efforts */
    ...
    if(!mode)prompt(SYSCOUNT1);
  } else if(!mode)prompt(SYSCOUNT,num_systems);
  ...
  if(usrlrm && !set_timeout){
    set_timeout=1;
    if(!mode)prompt(WARNTIME,usrlrm/60,msg_getunit(MINSNG,usrlrm/60));
  ...
  if((cl=clnt_create("vennea",METABBS_PROG,METABBS_VERS,"tcp"))==NULL){
    prompt(DNSFAIL);	/* The name's a bit misleading... */
    clnt_destroy(cl);
  }
  {
    ...
    print("--- RESULT=%d %-16.16s %s %s\n\n\n----------------------\n%s",
	  result->result_code,
	  result->club_header_u.club.club,
  ...
  }
  {
    ...
    if((result=distclub_request_message_1(&bbs,cl))==NULL){
      ...
      printf("compression: %d\n",result->club_message_u.message.comp_result.compression);
      printf("body len:    %d\n",result->club_message_u.message.message.message_len);
      printf("uncomp:      %d\n",
	     result->club_message_u.message.comp_result.compr_u.compr.orig_msg_len);
      printf("\nbody:\n");fflush(stdout);
      if(!result->club_message_u.message.comp_result.compression){
	write(1,result->club_message_u.message.message.message_val,
	      result->club_message_u.message.message.message_len);
      } else {
	char *tmp;
	int i=result->club_message_u.message.comp_result.compr_u.compr.orig_msg_len;
	tmp=alcmem(i);
	uncompress(tmp,
		   &i,
		   result->club_message_u.message.message.message_val,
		   result->club_message_u.message.message.message_len);
	write(1,tmp,i);
      }
      printf("<EOF>\n");fflush(stdout);
...
show_info(int i)
{
  ...
  if(strlen(p->bbs_ad))prompt(BBSAD,p->bbs_ad);
}
...
connect_with(int i)
{
  ...
  case -1:
    ...
    error_fatalsys("Unable to spawn telnet, dying in burning agony");
  ...
  if(p->disconnect==0)setchannelstate(LSR_LOGIN,"[NO-USER]");
  return p->disconnect;
}
