...
getline(int line)
{
  ...
  int i;
  if(line<=0)line=1;
  if(line>numlines)line=numlines;
...
gotoline(int line, int column)
{
  ...
  cy=i;
  current=l;
  if(column<0)cx=strlen(current->text);
  centerline();
}
...
movecursor(int dy,int dx)
{
  if(dx==-1){
    ...
    if(++cx>strlen(current->text)){
      if(cy<numlines)dx=-2;else cx=strlen(current->text);
      dy++;
  ...
  if(dy==-1){
    ...
    if(++cy>numlines+1)cy=numlines+1;
    if(cy==numlines+1){
      if(numbytes<maxsize){
	insertline(last,"");
	current=last;
    ...
    if(((signed)cy-(signed)((signed)LINES-3))>=(signed)toprow){
...
printansi(char *s)
{
  ...
  while ((c=*msgbuf++)!=0){
	...
	case 'B':
	  ...
	  if(!n)n++;
	  getyx(stdscr,cursory,cursorx);
	  move(min(LINES,cursory+n),cursorx);
	  break;
	case 'C':
	  n=atoi(parms);
	  if(!n)n++;
	  getyx(stdscr,cursory,cursorx);
	  move(cursory,min(COLS,cursorx+n));
	  break;
	case 'D':
	  n=atoi(parms);
	  if(!n)n++;
...
maskblock(int x1, int y1, int x2, int y2, unsigned char a)
{
  ...
  if(x1<0&&x2<0)return;
  if(x1>=COLS&&x2>=COLS)return;
  if(y1<0&&y2<0)return;
  if(y1>=LINES&&y2>=LINES)return;
  ...
  for(y=y1;y<=y2;y++){
    for(x=x1;x<=x2;x++){
      mvaddch(y,x,(unsigned char)(stdscr->_line[y].text[x]&0xff));
    }
  }
}
