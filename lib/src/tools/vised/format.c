...
formatline()
{
  ...
  if(!NOWRAP){
    ...
    *sp=0;
	...
	numbytes-=strlen(current->text);
	if(numbytes+strlen(temp)<=maxsize)strcpy(current->text,temp);
	else bel(ERRSIZ);
	numbytes+=strlen(current->text);
	showtext(cy-toprow+1);
...
justify(char *s)
{
  ...
  while(cp[strlen(cp)-1]==32)cp[strlen(cp)-1]=0;
  while(*cp==32)cp++;
  rmspaces(cp);
  len=rmargin-strlen(s);
  if((tp=strchr(cp,32))!=NULL)cp=tp;
  ep=strrchr(cp,32);
  if(!ep)return;
  l=strlen(cp);
  n=ep-cp+1;
  for(i=0;i<len;i++){
    ...
    ep=cp+t+i;
    if((sp=strchr(ep,32))==NULL)sp=strrchr(cp,32);
    if(!sp)return;
    c=*sp;
    *sp=0;
    strcpy(temp,cp);
...
formatwrap(int pass)
{
  ...
  oldcy=cy;
  while(cy<=numlines&&sameas(nohsp(current->text),""))current=getline(++cy);
  if(cy>numlines){
    cy=oldcy;
    return;
  }

  if(cy>1)tip=getline(cy-1);
  else {
...
/*	while((i>=0)&&(s[i]==32))s[i--]=0; */
	...
	l=getline(cy);
      ...
      strcpy(temp,l->text);
      if(formatmode==JUSTIFY)rmspaces(temp);
      line[0]=0;
      if(carry[0])sprintf(line,"%s ",carry);
      strcat(line,temp);
      if(temp[0]==0||sameas(temp," "))done=1;
      else deleteline(cy);
    } else if(!done && cy>numlines){
      done=1;
      continue;
    }else if(done&&carry[0])strcpy(line,carry);
	...
	if(new){
	  ...
	  strcpy(temp,line);
	  new->text=(char *)alcmem(MAXLEN);
	  if(formatmode==JUSTIFY)justify(temp);
      ...
      if(!c)strcpy(carry,cp+1);
      else sprintf(carry,"%c%s",c,cp+1);
    } else {
      strcpy(carry,line);
      if(done)break;
    }
...
formatflush(int pass)
{
  ...
  oldcy=cy;
  while(cy<=numlines&&sameas(current->text,""))current=getline(++cy);
  if(cy>numlines){
    cy=oldcy;
    return;
  }
  ...
  for(;cy<=numlines||carry[0];){
	...
	l=getline(cy);
      ...
      if(carry[0])sprintf(line,"%s ",carry);
      strcat(line,temp);
      if(line[0]==0||sameas(line," "))break;
      deleteline(cy);
    } else strcpy(line,carry);
	...
	dlines++;
      ...
      if(len && len<rmargin){
	memset(temp,32,sizeof(temp));
	strcpy(&temp[(rmargin-len)>>(formatmode==CENTRE)],line);
	strcpy(line,temp);
...
formatpara()
{
  ...
  if(numbytes>maxsize)bel(ERRFMT);
}
