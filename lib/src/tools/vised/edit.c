...
inschar (int c, int update)
{
  ...
  if(!insertmode){
    ...
    line[insp]=c;
    if(insp>=strlen(current->text))line[insp+1]=0;
    numbytes+=strlen(line);
    if(numbytes>maxsize){
    ...
    if(update)showtext(cy-toprow+1); 
    cx++;
    if(update){
      putcursor();
      refresh();
    ...
    if(strlen(current->text)>=rmargin /*&& NOWRAP*/){
      ...
      current->text[cx]=0;
      if(update)showtext(0);
    }
    strcpy(line,current->text);
    numbytes-=strlen(line);
    line[insp]=c;

    if(insp>=strlen(line))line[insp+1]=0;
    else strcpy(&line[insp+1],&current->text[insp]);
    numbytes+=strlen(line);
    if(numbytes>maxsize){
      numbytes=oldnumbytes;
      bel(ERRSIZ);
...
joinlines(int target)
{
  ...
  if(target<1||target==numlines)return;
  l1=getline(target);
  l2=getline(target+1);
