...
#include <megistos/mbk_stats.h>
#include <megistos/stats.h>
...
init ()
{
	...
	sprintf (oldstatsdir, "%s/%d", mkfname (STATDIR),
		 (int) tdyear (today ()));
...
mergedaystats (char *dir, char *name)
{
	...
	if ((out = fopen (fname1, "w")) == NULL)
		return;
	if ((in1 = fopen (mkfname (DAYSTATFILE), "a+")) == NULL)
		return;
	if ((in2 = fopen (fname2, "a+")) == NULL)
		return;
	rewind (in1);
	rewind (in2);
...
mergettystats (char *dir, char *name)
{
	...
	if ((out = fopen (fname1, "w")) == NULL)
		return;
	if ((in1 = fopen (mkfname (TTYSTATFILE), "a+")) == NULL)
		return;
	if ((in2 = fopen (fname2, "a+")) == NULL)
		return;
	rewind (in1);
	...
	while (!feof (in1)) {
		...
		if (numttystats >= MAXTTYS)
			break;
		if (i == 5) {
			memcpy (&ttystats[numttystats], &tty,
				sizeof (struct ttystats));
			numttystats++;
		} else
			break;
	}
	...
	while (!feof (in2)) {
		...
		if (i == 5) {
			...
			if (numttystats > 0) {
				for (j = 0; j < numttystats; j++)
					if (!strcmp
					    (ttystats[j].name, tty.name)) {
						ttystats[j].credits +=
						    tty.credits;
						ttystats[j].minutes +=
						    tty.minutes;
...
mergebaudstats (char *dir, char *name)
{
	...
	if ((out = fopen (fname1, "w")) == NULL)
		return;
	if ((in1 = fopen (mkfname (BAUDSTATFILE), "a+")) == NULL)
		return;
	if ((in2 = fopen (fname2, "a+")) == NULL)
		return;
	rewind (in1);
	...
	while (!feof (in1)) {
		...
		if (numbaudstats >= MAXBAUDS)
			break;
		if (i == 4) {
			memcpy (&baudstats[numbaudstats], &baud,
				sizeof (struct baudstats));
			numbaudstats++;
		} else
			break;
	}
	...
	while (!feof (in2)) {
		...
		if (i == 4) {
			...
			for (j = 0; j < numbaudstats; j++)
				if (baudstats[j].baud == baud.baud) {
...
mergeclsstats (char *dir, char *name)
{
	...
	if ((out = fopen (fname1, "w")) == NULL)
		return;
	if ((in1 = fopen (mkfname (CLSSTATFILE), "a+")) == NULL)
		return;
	if ((in2 = fopen (fname2, "a+")) == NULL)
		return;
	rewind (in1);
	...
	while (!feof (in1)) {
		...
		i = fscanf (in1, "%s %d %d\n", cls.name, &cls.credits,
			    &cls.minutes);
		if (numclsstats >= MAXCLSS)
			break;
		if (i == 3) {
			memcpy (&clsstats[numclsstats], &cls,
				sizeof (struct clsstats));
			numclsstats++;
		} else
			break;
	}
	...
	while (!feof (in2)) {
		...
		if (i == 3) {
			...
			for (j = 0; j < numclsstats; j++)
				if (!strcmp (clsstats[j].name, cls.name)) {
...
mergemodstats (char *dir, char *name)
{
	...
	if ((out = fopen (fname1, "w")) == NULL)
		return;
	if ((in1 = fopen (mkfname (MODSTATFILE), "a+")) == NULL)
		return;
	if ((in2 = fopen (fname2, "a+")) == NULL)
		return;
	rewind (in1);
	...
	while (!feof (in2)) {
		...
		if (i == 2) {
			...
			for (j = 0; j < nummodstats; j++)
				if (!strcmp (modstats[j].name, mod.name)) {
