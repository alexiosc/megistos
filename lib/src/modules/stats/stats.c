...
drawgraph(int type1, int type2, int type3, int type4)
{
  ...
  for(pass=0;pass<2;pass++){
	...
	break;
      case TTYSTATS:
	if(fscanf(fp,"%s %x %d %d %d\n",temp,&i,&field[0],&field[1],
		  &field[2])==5){
	  if(pass){
	    sprintf(label,"%03x",i);
	    grwid-=3;
	...
	break;
      ...
      barlen=field[type2];
      if(!pass){
	if(type1==AVERAGE)barlen/=numdays;
	if(barlen>max)max=barlen;
      } else {
	if(type1==AVERAGE)barlen/=numdays;
	grwid-=(numlen+1);
	barlen*=grwid*2;
	...
	prompt(GRBAR,label);
	sprintf(s,"%%-%dd",numlen+grwid-strlen(temp));
	total+=field[type2];
	if(type1==AVERAGE)field[type2]/=numdays;
...
demographics()
{
  ...
  prompt(AGESTATS,
	 ...
	 strtime(maketime(tm->tm_hour,tm->tm_min,tm->tm_sec),1),
	 ages[0][0],ages[1][0],ages[2][0],ages[3][0],ages[4][0],ages[5][0],
	 ages[0][1],ages[1][1],ages[2][1],ages[3][1],ages[4][1],ages[5][1],
	 ages[0][2],ages[1][2],ages[2][2],ages[3][2],ages[4][2],ages[5][2],
	 ages[0][3],ages[1][3],ages[2][3],ages[3][3],ages[4][3],ages[5][3],
	 ages[0][4],ages[1][4],ages[2][4],ages[3][4],ages[4][4],ages[5][4],
	 ages[0][5],ages[1][5],ages[2][5],ages[3][5],ages[4][5],ages[5][5],
	 ages[0][6],ages[1][6],ages[2][6],ages[3][6],ages[4][6],ages[5][6],
	 ages[0][7],ages[1][7],ages[2][7],ages[3][7],ages[4][7],ages[5][7],
	 ages[0][8],ages[1][8],ages[2][8],ages[3][8],ages[4][8],ages[5][8],
	 malestats,ANSIstats,visualstats,corpstats,suspstats,exemptstats,
	 sysaxstats,
	 langstats[0],langstats[1],langstats[2],langstats[3],langstats[4],
	 langstats[5],langstats[6],langstats[7],langstats[8],langstats[9]);
}
...
syntax()
{
  ...
  fprintf(stderr,"       -[ta][cml][bcdmt][dmye]: Create a graph, as follows:\n");
  ...
  fprintf(stderr,"         eg: -tcbm = BAUD stats: total credits consumed this month\n\n");
  exit(1);
}
...
main(int argc, char **argv)
{
  ...
  if(argc==2 && !strcmp(argv[1],"-cleanup")){
    ...
    if((type3!=BAUDSTATS && type3!=TTYSTATS) && type2==CALLS){
      fprintf(stderr,"statd: Impossible combination\n       (only baud- and ttystats provide CALL counts).\n");
      exit(1);
    }
