...
check(union action *a)
{
  ...
  case TYPE_SIMPLE:
    if(!a->s.simple[0]){
      fprintf(stderr,"Verb '%s' (S) has no 'simple' strings.\n",a->s.verb);
      exit(1);
    }
    break;
  case TYPE_OBJECT:
    if(!a->o.other[0]){
      fprintf(stderr,"Verb '%s' (O) has no 'other' strings.\n",a->o.verb);
      exit(1);
    } else if(!a->o.all[0]){
      fprintf(stderr,"Verb '%s' (O) has no 'all' strings.\n",a->o.verb);
      exit(1);
    } else if(!a->o.target[0]){
      fprintf(stderr,"Verb '%s' (O) has no 'target' strings.\n",a->o.verb);
      exit(1);
    } else if(!a->o.secret[0]){
      fprintf(stderr,"Verb '%s' (O) has no 'secret' strings.\n",a->o.verb);
      exit(1);
    }
    break;
  case TYPE_DOUBLE:
    if(!a->d.other[0]){
      fprintf(stderr,"Verb '%s' (D) has no 'other' strings.\n",a->d.verb);
      exit(1);
    } else if(!a->d.all[0]){
      fprintf(stderr,"Verb '%s' (D) has no 'all' strings.\n",a->d.verb);
      exit(1);
    } else if(!a->d.target[0]){
      fprintf(stderr,"Verb '%s' (D) has no 'target' strings.\n",a->d.verb);
      exit(1);
    } else if(!a->d.secret[0]){
      fprintf(stderr,"Verb '%s' (D) has no 'secret' strings.\n",a->d.verb);
      exit(1);
    } else if(!a->d.simple[0]){
      fprintf(stderr,"Verb '%s' (D) has no 'simple' strings.\n",a->d.verb);
      exit(1);
    }
    break;
  case TYPE_ADVERB:
    if(!a->a.adverb[0]){
      fprintf(stderr,"Verb '%s' (A) has no 'adverb' strings.\n",a->a.verb);
      exit(1);
    } else if(!a->a.simple[0]){
      fprintf(stderr,"Verb '%s' (A) has no 'simple' strings.\n",a->a.verb);
      exit(1);
    }
    break;
  case TYPE_GENERIC:
    if(!a->g.generic[0]){
      fprintf(stderr,"Verb '%s' (G) has no 'generic' strings.\n",a->a.verb);
      exit(1);
    }
    break;
  }
...
expand(char **s)
{
  ...
  for(i=1;i<ML;i++){
    if(s[i]==NULL || strlen(s[i])==0)s[i]=prev;
    prev=s[i];
  }
}
...
{
  int i;
  for(i=0;i<verbs;i++)if(sameas(actions[i].s.verb,verb))return 1;
  return 0;
}
...
parsein(int num,char *keyword,char *array[],char *data)
{
  ...
  if(n>=ML){
    fprintf(stderr,"Line %d: more than %d '%s' lines in this def.\n",
	    num,ML,keyword);
    exit(1);
  } else array[n]=strdup(data);
}
...
main(int argc, char *argv[])
{
  ...
  while(!feof(fin)){
    ...
    if((cp=strrchr(line,'\n'))!=NULL)
      for(;(*cp=='\n')||(*cp=='\r')||isspace(*cp);cp--)*cp=0;
    for(cp=line;isspace(*cp);cp++);
    if(*cp=='#')continue;
    if(!strlen(cp))continue;
    
    if((sscanf(cp,"%s %n",keyword,&n)!=1))continue;

    data=&cp[n];

    if((k=bsearch(keyword,keywords,nkeywords,sizeof(struct keyword),keycmp))==NULL){
	...
	fprintf(stderr,"Line %d: bad format for 'verb'.\n",num);
	exit(1);
      } else if(!strchr(ACTIONTYPES,a.s.type=toupper(a.s.type))){
	fprintf(stderr,"Line %d: bad verb type '%c'.\n",num,a.s.type);
	exit(1);
      } else if(verbexists(tmp)){
	fprintf(stderr,"Line %d: duplicate verb '%s'.\n",num,tmp);
	exit(1);
      }
      a.s.verb=strdup(tmp);
      break;
    ...
    case CODE_SIMPLE:
      if(a.s.type==TYPE_SIMPLE)parsein(num,"simple",a.s.simple,data);
      else if(a.s.type==TYPE_DOUBLE)parsein(num,"simple",a.d.simple,data);
      else if(a.s.type==TYPE_ADVERB)parsein(num,"simple",a.a.simple,data);
      else {
	fprintf(stderr,"Line %d: can't use 'simple' for this verb type.\n",
		num);
	exit(1);
      }
      break;
    ...
    case CODE_OTHER:
      if(a.s.type==TYPE_OBJECT)parsein(num,"other",a.o.other,data);
      else if(a.s.type==TYPE_DOUBLE)parsein(num,"other",a.d.other,data);
      else {
	fprintf(stderr,"Line %d: can't use 'other' for this verb type.\n",
		num);
	exit(1);
      }
      break;
    ...
    case CODE_ALL:
      if(a.s.type==TYPE_OBJECT)parsein(num,"all",a.o.all,data);
      else if(a.s.type==TYPE_DOUBLE)parsein(num,"all",a.d.all,data);
      else {
	fprintf(stderr,"Line %d: can't use 'all' for this verb type.\n",
		num);
	exit(1);
      }
      break;
    ...
    case CODE_TARGET:
      if(a.s.type==TYPE_OBJECT)parsein(num,"target",a.o.target,data);
      else if(a.s.type==TYPE_DOUBLE)parsein(num,"target",a.d.target,data);
      else {
	fprintf(stderr,"Line %d: can't use 'target' for this verb type.\n",
		num);
	exit(1);
      }
      break;
    ...
    case CODE_SECRET:
      if(a.s.type==TYPE_OBJECT)parsein(num,"secret",a.o.secret,data);
      else if(a.s.type==TYPE_DOUBLE)parsein(num,"secret",a.d.secret,data);
      else {
	fprintf(stderr,"Line %d: can't use 'secret' for this verb type.\n",
		num);
	exit(1);
      }
      break;
    ...
    case CODE_ADVERB:
      if(a.s.type==TYPE_ADVERB)parsein(num,"adverb",a.a.adverb,data);
      else {
	fprintf(stderr,"Line %d: can't use 'adverb' for this verb type.\n",
		num);
	exit(1);
      }
      break;
    ...
    case CODE_GENERIC:
      if(a.s.type==TYPE_GENERIC)parsein(num,"generic",a.g.generic,data);
      else {
	fprintf(stderr,"Line %d: can't use 'generic' for this verb type.\n",
		num);
	exit(1);
      }
      break;
    ...
    case CODE_ACCESS:
      {
	char tmp2[16384];
	if(a.s.access)strcpy(tmp2,a.s.access);
	else tmp2[0]=0;
	for(cp=data;*cp;cp++)*cp=toupper(*cp);
	cp=strtok(data," \t");
	while(cp){
	  sprintf(tmp,"(%s)",cp);
	  strcat(tmp2,tmp);
	...
	fprintf(stderr,"Line %d: bad format for 'key'.\n",num);
	exit(1);
      } else if(a.s.key<0||a.s.key>129){
	fprintf(stderr,"Line %d: key must be 0-129.\n",num);
	exit(1);
      }
      break;
    }
