...
startchat()
{
  ...
  for(;othruseraux.chatting&&state!=3;){
    ...
    if((n=read(fileno(stdin),s,sizeof(s)-1))>0){
      ...
      if(setcolour!=0){
	setcolour=0;
	sprintf(msg.s,"\033[%sm",colours[thisuseraux.colour]);
	...
	int i,j;
	for(i=0,j=0;i<n;i++,j++){
	  if(!state&&((s[i]==13)||s[i]==10)){
	    s[i]='\n';
	    state=1;
#ifdef GREEK
	  } else if(state==1&&(s[i]==-82||s[i]==-107||toupper(s[i])=='X')){
#else
 	  } else if(state==1&&(toupper(s[i])=='X')){
#endif
	    state=2;
	  } else if(state==2&&((s[i]==13)||s[i]==10)){
	    state=3;
	  } else state=0;
    ...
    if((n=msgrcv(thisuseraux.chatid,(struct msg_buffer*)&msg,sizeof(msg.s)-1,
      ...
      if(setcolour!=1){
	setcolour=1;
	sprintf(s,"\033[%sm",colours[othruseraux.colour]);
...
finishchat()
{
  ...
  setusrax(curchannel,thisuseracc.userid,0,CUF_PRESENT,CUF_CHATTING);
  ...
  if(loadtlcuser(thisuseracc.userid,&tlcu))thisuseraux.colour=tlcu.colour;
