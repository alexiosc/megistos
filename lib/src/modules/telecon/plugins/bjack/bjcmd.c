...
void bj_parse_command(char *s, enum bj_cmd *cmd, int *i)
{
  char ss[50];
	...
	sscanf(s, "%s", ss);
	if (!strcasecmp(ss, "on")) { *cmd = BJ_ON; if(sscanf(s, "%*s %i", i)==EOF) *i=-1; return; }
	if (!strcasecmp(ss, "off")) { *cmd = BJ_OFF; return; }
	if (!strcasecmp(ss, "exit")) { *cmd = BJ_EXIT; return; }
	if (!strcasecmp(ss, "bet")) { *cmd = BJ_BET; if(sscanf(s, "%*s %i", i)==EOF) *i=-1; return; }
	if (!strcasecmp(ss, "raise")) { *cmd = BJ_RAISE; if(sscanf(s, "%*s %i", i)==EOF) *i=-1; return; }
	if (!strcasecmp(ss, "call")) { *cmd = BJ_CALL; return; }
	if (!strcasecmp(ss, "fold")) { *cmd = BJ_FOLD; return; }
	if (!strcasecmp(ss, "pot")) { *cmd = BJ_POT; return; }
	if (!strcasecmp(ss, "hit")) { *cmd = BJ_HIT; return; }
	if (!strcasecmp(ss, "stay")) { *cmd = BJ_STAY; return; }
	if (!strcasecmp(ss, "hand")) { *cmd = BJ_HAND; return; }
	if (!strcasecmp(ss, "turn")) { *cmd = BJ_TURN; return; }
	if (!strcasecmp(ss, "help")) { *cmd = BJ_HELP; return; }
	if (!strcasecmp(ss, "rules")) { *cmd = BJ_RULES; return; }
	if (!strcasecmp(ss, "info")) { *cmd = BJ_INFO; return; }
	if (!strcasecmp(ss, "scan")) { *cmd = BJ_SCAN; return; }
	if (!strcasecmp(ss, "bell")) { *cmd = BJ_BELL; if(sscanf(s, "%*s %i", i)==EOF) *i=-1; return; }
	
#ifdef CHEATS
	if (!strcasecmp(ss, "showdeck")) { *cmd = BJ_SDECK; return; }
...
#ifdef DEVEL
	if (!strcasecmp(ss, "start")) { *cmd = BJ_START; return; }
	if (!strcasecmp(ss, "debug")) { *cmd = BJ_DEBUG; if (sscanf(s, "%*s %i", i)==EOF) *i=-1; return; }
#endif
...
{
#ifdef COMMAND_LOG
	bj_logmsg("- Status %s executing %s (i=%i)", stat_names[ STATUS ], cmd_names[cmd], i);
#endif

/* if user is not registered then he is only allowed to use some specific commands */
	if ((player==NULL) && (!(
		(cmd==BJ_ON) || (cmd==BJ_HELP) || (cmd==BJ_RULES) || (cmd==BJ_INFO) || (cmd==BJ_DEBUG)
...
/* miscellaneous commands */
	case BJ_ON: bj_on(player, i); break;
	case BJ_OFF: bj_off(player); break;

	case BJ_EXIT: break;
	case BJ_HELP: bj_broadcast_msg(BJM_HELP, 0, bj_userid); break;
	case BJ_RULES: bj_broadcast_msg(BJM_RULES, 0, bj_userid, P_MIN_PLAYERS, P_PAUSE_EXP,
				P_TICK_EXP, P_WARN_EXP);
		break;
	case BJ_INFO: bj_broadcast_msg(BJM_INFO, 0, bj_userid, MODULE_ID); break;

	case BJ_POT: bj_pot(player); break;
	case BJ_TURN: bj_turn(player); break;
	case BJ_SCAN: bj_scan(bj_userid /*player*/); break;
	case BJ_HAND: bj_hand(player); break;

	case BJ_BET: if (bj_bet(player, i)) bj_next_player(); break;
	case BJ_RAISE: if (bj_raise(player, i)) bj_next_player(); break;
	case BJ_CALL: if (bj_call(player)) bj_next_player(); break;
	case BJ_FOLD: if (bj_fold(player)) bj_next_player(); break;
	case BJ_HIT: if (bj_hit(player)) bj_next_player(); break;
	case BJ_STAY: if (bj_stay(player)) bj_next_player(); break;

	case BJ_BELL: {
			if (player->flags&bjfPFIX) player->flags  &= ~bjfPFIX;
			else player->flags |= bjfPFIX;
			
			bj_broadcast_msg(BJM_BELL, 0, player->userid, bj_get_pfix(0, BJM_BELLACT, (player->flags&bjfPFIX)==0));
		}
		break;
						
#ifdef CHEATS
	case BJ_SDECK: bj_show_deck(player); break;
	case BJ_GIVE_CARD: bj_give_card(player, i); break;
#endif


#ifdef DEVEL
	case BJ_START: if (STATUS==STAT_NOGAME) STATUS=STAT_NEWGAME; break;
	case BJ_DEBUG: bj_debug(bj_userid, i); break;
#endif
...
void bj_on(struct bj_player *player, int i)
{
  struct bj_player *pl;
	...
	if(i==1) bj_execute_command(pl, BJ_BELL, 0);
}
  ...
  struct bj_player *pl, *prev;
	...
	do {
		if (!strcmp(pl->userid, player->userid)) {
			...
			if ((c_player!=NULL) && (!strcmp(player->userid, c_player->userid)))
				bj_next_player();
...
int bj_bet(struct bj_player *player, int bet)
{
	...
	if (((bj_player_credits(player) - bet) >= P_MIN_CREDITS)
		...
		if ((player->bet+bet) >= current_bet) {
			...
			bj_broadcast_msg(BJM_YBETCRED, BJM_OBETCRED, player->userid, bet,
					bj_get_pfix(0, BJM_MPLAYER, 
					(player->flags&bjfSEX)), player->userid, bet);
			
			return 1;

		} else
			bj_broadcast_msg(BJM_MINBET, 0, player->userid, current_bet-player->bet);
...
int bj_call(struct bj_player *player)
{
  int bt;
	...
	if (((bj_player_credits(player) - (current_bet - player->bet)) >= P_MIN_CREDITS)
		|| (P_SYSOPS_NOCHARGE && (player->flags&bjfNOCHARGE))) {
		...
		bj_broadcast_msg(BJM_YCALL, BJM_OCALL, player->userid, 
				bj_get_pfix(0, BJM_MPLAYER, (player->flags&bjfSEX)), player->userid);
...
void bj_turn(struct bj_player *player)
{
	...
	if (c_player==player)
		...
		bj_broadcast_msg(BJM_OTURN, 0, player->userid, c_player->userid);
}
  ...
  char buffer[2048], buf[256], buf1[10];;
	...
	while (pl != NULL) {
		...
		sprintf(buf, bj_get_prompt( BJM_SCANREC ), pl->userid, buf1,
			(pl->flags&bjfPLAY)?'*':' ', (pl->flags&bjfSTAY)?'+':' ');
		strcat(buffer, buf);
...
int bj_card_sum(struct bj_player *player)
{
  int i, sum, ace;
  char t1, t2;
...
int bj_get_rand(int min, int max)
{
	return ((int)((float)(max-min)*rand() / ((float)RAND_MAX+1.0)))+min;
}
  ...
  char buffer[512];
	...
	while(pl != NULL) {
		if (pl->flags&bjfWIN) {
			...
			sprintf(buffer, "%s %s %s %s", buffer, ((j>1)?((j==i)?bj_get_prompt(BJM_AND):"\b,"):"\b"),
					bj_get_pfix(0, BJM_SMTHE, (pl->flags&bjfSEX)), pl->userid);
		}
	...
	bj_broadcast_msg(0, BJM_WINNERS, "", 
			bj_get_pfix(0, BJM_SWON, (j==1)), buffer, card_sum);
...
void shuffle_deck(void)				/* structure from Major */
{
  int i, temp, swap;
