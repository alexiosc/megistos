 ...
 *
 * $Log$
 * Revision 1.1  2001/04/16 15:00:06  alexios
 * Initial revision
 *
...
struct bj_player *bj_ingame(char *bjuser)
{
  struct bj_player *pl;
	...
	pl = player_list;
	while(pl!=NULL){ 
		if(!strcasecmp(pl->userid, bjuser)) return pl;
		pl=pl->next;
	}
...
/* we can exit loop and return c_player iff : we are STAT_BETTING and returned to the original 
   user or c_player must bet (player->bet < current_bet) */
		...
		if ((STATUS == STAT_BETTING) && ((c_player->flags&bjfPLAY)
					 || ((c_player == pl) && (c_player->bet < current_bet))))
			break;
...
/* if all the still playing players have stayed search for winner */
		if ((STATUS == STAT_HITTING) && (stay_count==play_count)) break;
		
/* all the players have quited! */
		if ((STATUS == STAT_BETTING) && (!play_count)) break;
...
/* if we are STAT_BETTING and c_player->bet==current_bet and current_bet!=0 then
	...
	if ((c_player->bet == current_bet) && (STATUS == STAT_BETTING) && (current_bet)) {
...
struct bj_player *bj_new_player(char *userid, int cred)
{
  struct bj_player *pl;
	...
	if (pl==NULL) {
		...
		bj_logmsg("ERROR: Could not allocate memory for player %s", userid);
...
void bj_loop(void)
{
  ...
  do {
    ...
    if ((user_advanced()) && (c_player!=NULL)) {
      if (STATUS==STAT_BETTING) bj_broadcast_msg(BJM_YTURNBET, 0, c_player->userid);
      else if (STATUS==STAT_HITTING) bj_broadcast_msg(BJM_YTURNHIT, 0, c_player->userid);
    }
    ...
    */
	...
	tick_timer = time(0) + P_TICK_EXP;
      ...
      if ((cmd==BJ_EXIT) && (!strcasecmp(bj_userid, "valis"))) {
	bj_broadcast_msg(0, BJM_STRING, "", "--- DEBUG: Finishing BLACKJACK\n");
	break;
      } else cmd=BJ_NO_CMD;
    ...
    if (time(0)>watchdog_timer) {
      ...
      bj_logmsg("Long inactivity period - plugin commits suicide");
      break;
    }
    
    if ((time(0)>tick_timer) && ((STATUS==STAT_BETTING) || (STATUS==STAT_HITTING))) {
      if (c_player!=NULL) {
	tick_warn++;
	if (tick_warn>P_WARN_EXP) {
	  
	  bj_broadcast_msg(BJM_WARNEXP, 0, c_player->userid, 
			   bj_get_pfix(0, BJM_MINACTIVE, (c_player->flags&bjfSEX)));
	  
	  
	  
	  if (c_player->flags&bjfFORCE) {
	    bj_broadcast_msg(0, BJM_OFORCEOFF, "",
			     bj_get_pfix(0, BJM_MPLAYER, (c_player->flags&bjfSEX)), c_player->userid);
	    
#ifdef FORCE_LOG
	    bj_logmsg("*FORCE* Player %s forced to quit", c_player->userid);
#endif
	  ...
	  switch(STATUS) {
	  case STAT_BETTING: 
	    bj_broadcast_msg(BJM_YFORCEFOLD, BJM_OFORCEFOLD, 
			     c_player->userid, bj_get_pfix(0, BJM_MPLAYER, (c_player->flags&bjfSEX)), 
			     c_player->userid,
			     bj_get_pfix(1, BJM_MINACTIVE, (c_player->flags&bjfSEX)));
	    
	    bj_fold(c_player); 
#ifdef FORCE_LOG
	    bj_logmsg("*FORCE* Player %s forced to fold", c_player->userid);
#endif
	    break;
	    
	  case STAT_HITTING: 
	    bj_broadcast_msg(BJM_YFORCESTAY, BJM_OFORCESTAY,
			     c_player->userid, bj_get_pfix(0, BJM_MPLAYER, (c_player->flags&bjfSEX)),
			     c_player->userid,
			     bj_get_pfix(1, BJM_MINACTIVE, (c_player->flags&bjfSEX)));
	    ...
	    bj_stay(c_player); 
#ifdef FORCE_LOG
	    bj_logmsg("*FORCE* Player %s forced to stay", c_player->userid);
#endif
	    break;
	  }
	...
	bj_broadcast_msg(BJM_TICK, 0, c_player->userid);
	tick_timer = time(0) + P_TICK_EXP;
	advance_user=1;
      }
    }
	...
	&& (((pause_timer-time(0))%P_PAUSE_WARN==0) || (cmd==BJ_ON))) {
      
      /* inform every other user that joins about the time... */
      if (cmd==BJ_ON) temp=0;
      
      if ((!temp) && (CUR_PLAYERS<P_MIN_PLAYERS)) {
	bj_broadcast_msg(0, BJM_WAITMORE, "", P_MIN_PLAYERS);
	temp=1;
	pause_timer = time(0) + P_PAUSE_EXP;
	continue;
      }
	...
	if (pause_timer<=time(0))
	  ...
	  if (P_EPAUSE_WARN)
	    bj_broadcast_msg(0, BJM_START, "", pause_timer-time(0));
	
	temp=1;
      }
    } else temp=0;
    
    if ((pause_timer) && (time(0)>pause_timer) && (STATUS==STAT_NOGAME))
      STATUS=STAT_NEWGAME;
...
void _bj_logmsg(char *file, int line, char *func, char *format, ...)
{
  ...
  char datetime[64];
  	...
  	if ((fp=fopen(LOGFILE, "a"))==NULL)return;
	t=time(0);
	dt=localtime(&t);
	strftime(datetime,sizeof(datetime),"%d/%m/%Y %H:%M:%S",dt);

#ifdef VERBOSE_LOG
	fprintf(fp,"%s [%s:%d %20s] {%s} (%s): ", datetime,file,line, func, bj_channel, ((bj_userid[0]==0)?"unknown":bj_userid));
#else
	fprintf(fp,"%s [%s:%d] {%s} (%s): ", datetime,file,line, bj_channel, ((bj_userid[0]==0)?"unknown":bj_userid));
#endif
