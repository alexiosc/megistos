...
addqsc(char *club, int lastmsg, int flags)
{
  ...
  if((p=findqsc(club))==NULL){
    memset(&x,0,sizeof(x));
    strcpy(x.club,club);
    if(lastmsg>=0)x.lastmsg=x.emllast=lastmsg;
    if(flags&LRF_INITQWK)x.qwklast=lastmsg;
    x.flags=flags;
    ...
    if(lastmsg>=0)p->lastmsg=p->emllast=lastmsg;
    if(flags&LRF_INITQWK)x.qwklast=lastmsg;
    p->flags&=~flags;
    p->flags|=flags;
  }
...
all(int add)
{
  ...
  for(i=0;i<n;free(clubs[i]),i++){
    ...
    if(add==DEL){
      ...
      addqsc(clubhdr.club,0,p?p->flags|LRF_QUICKSCAN:LRF_QUICKSCAN);
    }
  }
  free(clubs);
  enterdefaultclub();
...
initialise()
{
  ...
  for(i=0;i<n;free(clubs[i]),i++){
    char *cp=&clubs[i]->d_name[1];
    if(!loadclubhdr(cp))continue;
    addqsc(clubhdr.club,clubhdr.msgno,LRF_QUICKSCAN|LRF_INQWK|LRF_INITQWK);
  }
  free(clubs);
  enterdefaultclub();
}
...
setlastread(char *club, int msg)
{
  ...
  if(!p)addqsc(club,msg,0);
  else p->lastmsg=p->emllast=msg;
  saveqsc();
}
...
listqsc()
{
  ...
  p=resetqsc();
  while(p){
    if((p->flags&LRF_QUICKSCAN)&&(getclubax(&thisuseracc,p->club)>CAX_ZERO)){
      if(first){
	prompt(RQCLH);
      }
      first=0;
...
addclub()
{
  ...
  if(!getclub(club,RQCADD,RQCDR,1))return;
  if(sameas(club,"ALL"))all(ADD);
  else {
    loadclubhdr(club);
    p=findqsc(club);
    if(!p)addqsc(club,0,LRF_QUICKSCAN);
    else if(p->flags&LRF_QUICKSCAN)addqsc(club,clubhdr.msgno,p->flags);
    else addqsc(club,0,p->flags|LRF_QUICKSCAN);
  }
}
...
delclub()
{
  ...
  for(;;){
    if(!getclub(club,RQCDEL,RQCDR,1))return;
    if(sameas(club,"ALL")){
      all(DEL);
      return;
...
doquickscan()
{
  ...
  selclub:
    ...
    if(inemail==OPT_ALL)if((lastread->flags&LRF_QUICKSCAN)==0)continue;
    ...
    if(!keyscan)prompt(QSSCAN,lastread->club);
    else showkeywords();
	...
	inemail==OPT_ALL?lastread->lastmsg+1:lastread->emllast+1;
    }
    j=findmsgnum(&msgno,dir==BSD_LT?LASTMSG:lastread->entrymsg,dir);
    if(j!=BSE_FOUND || msgno<0){
      lastread->entrymsg=-1;
      continue;
    } else lastread->entrymsg=msgno;
