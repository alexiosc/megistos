...
list_clubs(char *fname, char *sysname)
{
  ...
  *cp++=0;			/* tmp=hostname, cp=codename */
  ...
  if(debug)fprintf(stderr,"Sending club list request to %s/%s\n",tmp,cp);
  ...
  if(!dryrun){
    ...
    while(!feof(fp)){
      ...
      if(!sameto("club: ",line)) fprintf(out,"%s",line);
    }
    ...
    if(rename(fname2,fname)<0){
      int i=errno;
      fprintf(stderr,"Unable to rename the import point file! errno=%d (%s)\n",
	      i,strerror(i));
    }
  }
