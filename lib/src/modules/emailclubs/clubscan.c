...
{
  stgxlate(s,KEYSCAN);
  for(lastkey=0;lastkey<numkeys;lastkey++)if(strstr(s,phonetic[lastkey])){
    return 1;
  }
  return 0;
}
...
scanupdatemsg(struct message *msg,int read)
{
  ...
  if(inemail==OPT_ALL)l->lastmsg=max(m.msgno,l->lastmsg);
  else if(inemail==OPT_TOYOU)l->emllast=max(m.msgno,l->emllast);
...
locatemsg(int *msgno, int *sequencebroken, int targetmsg, int dir, int mode)
{
  ...
  if(*sequencebroken){
	...
	j=findmsgto(msgno,thisuseracc.userid,targetmsg,tdir);
      else j=findmsgnum(msgno,targetmsg,tdir);
	...
	else findmsgnum(msgno,*msgno,dir);
      }
    }
    ...
    *sequencebroken=0;
    ...
    else j=npmsgnum(msgno,targetmsg,dir);
  }
...
scan4files(int *msgno, int *sequencebroken, int targetnum, int dir, int mode)
{
  ...
  *msgno=-1;
  for(;;){
    ...
    if(filescan==2){
      found=((idx.flags&MSF_FILEATT)&&((idx.flags&MSF_APPROVD)==0));
    } else found=idx.flags&MSF_FILEATT;
...
scan4msg(int *msgno, int *sequencebroken, int targetnum, int dir, int mode)
{
  ...
  if(!keyscan){
    if(!filescan)return locatemsg(msgno,sequencebroken,targetnum,dir,mode);
    else return scan4files(msgno,sequencebroken,targetnum,dir,mode);
  } else {
    struct message msg;
    int first=1;
    ...
    for(;;){
      ...
      *msgno=-1;
      if(!filescan)j=locatemsg(msgno,sequencebroken,targetnum,dir,mode);
      else j=scan4files(msgno,sequencebroken,targetnum,dir,mode);
	...
	*msgno=-1;
	...
	first=0;
      ...
      if(keycmp(msg.from))found=1;
      else if(keycmp(msg.to))found=1;
      else if(keycmp(msg.subject))found=1;
      else if(keycmp(msg.history))found=1;
      else if(keycmp(msg.fatt))found=1;
      else if(keycmp(msg.origbbs))found=1;
      else if(keycmp(msg.origclub))found=1;
      else if(keycmp(msg.msgid))found=1;
      else {
	FILE *fp;
	char fname[256], line[1024];
	
	sprintf(fname,"%s/%s/"MESSAGEFILE,mkfname(MSGSDIR),msg.club,msg.msgno);
...
startscanning(int startmsg, int bdir)
{
  ...
  for(i=startmsg;;){
    ...
    oldmsgno=msgno;
    msgno=-1;
    if(!threadmessage)setclub(clubhdr.club);
    else setclub(threadclub);
	...
	if(isfirstqsc() && dir==BSD_LT){
	  ...
	  prompt(RSEND);
	  ret=0;
	} else if(nummsgs)prompt(QSLEAVE);
	if(ret)return dir==BSD_LT?BSE_BEGIN:BSE_END;
      }
      bound=dir;
      defopt='X';
      dontshow=1;
	...
	if(msgno<=targetmsg){
	  saveqsc();
	  return dir==BSD_LT?BSE_BEGIN:BSE_END;
	}
      } else {
    ...
    if(threadmessage)prompt(THREAD);
  ...
  bound_hit:
      ...
      case '#':
	{
      ...
      case 'T':
	{
	  int newmsgno=-thread(&msg,dir==BSD_GT?'F':'B');
	  if(newmsgno>0){
	    i=msgno=newmsgno;
	    dontshow=0;
	  } else {
	...
	scanupdatemsg(&msg,1);
	do{
	  opt=clubreadmenu(&msg,dir==BSD_GT?'N':'P');
	  optdecided=1;
	...
	return 'p';
      ...
      case 'b':
	if(inemail==OPT_ALL)lastread->lastmsg=msg.msgno-1;
	else if(inemail==OPT_TOYOU)lastread->emllast=msg.msgno-1;
	saveqsc();
	prompt(QSMBM,msg.msgno,clubhdr.club);
	return 'b';

      case 'c':
	if(inemail==OPT_ALL)lastread->lastmsg=clubhdr.msgno;
	else if(inemail==OPT_TOYOU)lastread->emllast=clubhdr.msgno;
	saveqsc();
...
scanmessages()
{
  ...
  if(!quickscan){
    ...
    if(!getrdmsgno(&startmsg,RSASK,RSASKH,RSASKR,getlastread(clubhdr.club))){
      doneqsc();
      return;
    }
    ...
    msgno=-1;
    j=scan4msg(&msgno,&sequencebroken,startmsg,BSD_GT,inemail);
    if(j==BSE_CAN)return;
    if(msgno<0 || j!=BSE_FOUND){
      sequencebroken=1;
      j=scan4msg(&msgno,&sequencebroken,startmsg,BSD_LT,inemail);
      if(j==BSE_CAN)return;
      if(msgno<0||j!=BSE_FOUND){
	prompt(RSBNMSG);
	cnc_end();
  ...
  else startscanning(msgno,BSD_GT);
  keyscan=0;
  saveqsc();
