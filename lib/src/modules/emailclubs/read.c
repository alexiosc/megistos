...
readmenu (message_t *msg, char defopt)
{
	...
	for (;;) {
		...
		res =
		    get_menu (&opt, 1, 0, menu, RDMNUR, options, RDDEF,
			      defopt);
...
headermenu (char defopt)
{
	...
	for (;;) {
		inp_setflags (INF_HELP);
		res =
		    get_menu (&opt, 1, 0, RDSCNM, RDSCNR, "NP#RCX", RDDEF,
			      defopt);
		inp_clearflags (INF_HELP);
...
emailscan (int reademail, int readopt, int startmsg)
{
	...
	if (reademail)
		res = startreading (readopt, startmsg);
	if (res != 'C')
		return;
	...
	do {
		...
		if (l == NULL)
			break;
		if (getclubax (&thisuseracc, l->club) < CAX_READ)
			continue;
		count++;
	} while (nextqsc ());
	doneqsc ();
...
emailread ()
{
	...
	if (!get_menu (&opt, 1, 0, RDWHAT, RDWHATR, "TPF", 0, 0))
		return;
	switch (opt) {
	case 'T':
		readopt = 0;
	...
	if (readopt == OPT_FROMYOU) {
		j = findmsgfrom (&msgno, thisuseracc.userid,
				 ecuser.lastemailread + 1, BSD_GT);
		if (j != BSE_FOUND) {
			j = findmsgfrom (&msgno, thisuseracc.userid,
					 ecuser.lastemailread + 1, BSD_LT);
		}
	} else {
		j = findmsgto (&msgno, thisuseracc.userid,
			       ecuser.lastemailread + 1, BSD_GT);
	...
	if (!getrdmsgno (&startmsg, RDASKNO, RDASKH, RDASKNOR, msgno))
		return;
