...
mkparentlib(char *libname, char *dirname, int key)
{
  ...
  if(res!=1){
    ...
    if(dirname!=NULL)strcpy(lib.dir,dirname);
    else sprintf(lib.dir,"%s/%s",library.dir,libname);
    ...
    lib.readkey=lib.downloadkey=lib.uploadkey=lib.overwritekey=key;
...
void
libsig(char *arg_under, char *arg_basedir,
	    char *arg_majordir, int arg_key)
{
  ...
  if(fp==NULL){
    ...
    if((fp=fopen(fname,"r"))==NULL){
      fprintf(stderr,"libcnv: libsig(): unable to find libsig.txt or LIBSIG.TXT.\n");
      exit(1);
    }
  }

  mkparentlib(arg_under==NULL?"Major":arg_under,arg_basedir,arg_key);
  printf("Installing under %s\n",library.fullname);

  while(!feof(fp)){
    if(fscanf(fp,"%d\n",&reclen)==0){
      if(feof(fp))break;
      fprintf(stderr,"libcnv: libsig(): unable to parse record length. Corrupted file?\n");
      exit(1);
    }
    if(feof(fp))break;
    ...
    if(fread(rec,reclen,1,fp)!=1){
      fprintf(stderr,"libcnv: libsig(): unable to read record around pos=%ld.\n",ftell(fp));
      exit(1);
    }
    ...
    /* Access keys */
    lib.readkey=lib.downloadkey=lib.uploadkey=lib.overwritekey=arg_key;
    ...
    /* FLAGS */
    {
      ...
      int newflags=0;
      if(flags&0x100) newflags|=LBF_READONLY; /* Read-only LIB */
      if(flags&0x200) newflags|=LBF_NOINDEX; /* Don't make indices */
      if(flags&0x400) newflags|=LBF_DNLAUD; /* Audit downloads */
      if(flags&0x800) newflags|=LBF_UPLAUD; /* Audit uploads */
      if((flags&0x1000)==0) newflags|=LBF_LOCKUPL; /* Password on uploads */
      if((flags&0x2000)==0) newflags|=LBF_LOCKDNL; /* Passwd on downloads */
      if(flags&0x8000) newflags|=LBF_OSDIR; /* OS LIB */
      lib.flags=newflags;
    }

    {
      struct libidx tmplib;
      if(libread(lib.keyname, library.libnum, &tmplib)==1){
	printf("Warning: library %s already exists. Will install to it anyway.\n",
	       lib.fullname);
      } else mklib(&lib,1,0);
    }
