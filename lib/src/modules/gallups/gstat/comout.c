  ...
  int siz0, siz1;
	...
	for(i=0;i<siz0;i++) {
		...
		for(j=0;j<siz1;j++) {
			...
			for(z=0;z<zm;z++) {
				key[4]=z;
				CA.fields[(i*zm+z)*siz1+j] = class_sum(key, INTmask);
			}
		}
	}
...
void dump_combo_options_sex_age(void)
{
  ...
  int siz0, siz1, siz2;
	...
	for(i=0;i<siz0;i++) {
		...
		for(n=0;n<siz1;n++) {
			...
			for(j=0;j<siz2;j++) {
				...
				for(z=0;z<zm;z++) {
					key[4]=z;
					CA.fields[((i*zm+z)*siz1+n)*siz2+j] = class_sum(key, INTmask);
				}
			}
		}
	}
	...
	for(i=0;i<siz0;i++) {
		CA.comboidx=0;
		for(z=0;z<zm;z++) {
			load_ca(i, (i*zm+z)*siz1*siz2, (i*zm+z)*siz1, 0);
			dump(TEMT_COSA);
		}
		if(i<siz0-1)dump( TEMS_COSA );
	}
  ...
  treekey_t key;
	...
	for(i=0;i<siz0;i++) {
		...
		for(z=0;z<zm;z++) {
			...
			j = floor((WIDTH * CA.fields[i*zm+z] / CA.fld_p_sum[i]) + 0.5);
	...
	for(i=0;i<qseldatacnt(curquest);i++) {
		...
		for(z=0;z<zm;z++) {
			...
			for(jj=0;jj<siz1;jj++) {
				...
				j = floor((WIDTH * CA.fields[(i*zm+z)*siz1+jj] / CA.fld_p_sum[i]) + 0.5);
				for(;j>0;j--,n--)strcat(tempstr, BARCHAR);
			}

			if(WIDTH - n < (WIDTH * CA.row_p_sum[i*zm+z] / CA.fld_p_sum[i])) {
	...
	for(i=0;i<siz0;i++) {
		...
		for(z=0;z<zm;z++) {
			...
			for(k=0;k<siz1;k++) {
				...
				for(j=0;j<siz2;j++) {
					jj = floor((WIDTH * CA.fields[((i*zm+z)*siz1+k)*siz2+j] / CA.fld_p_sum[i]) + 0.5);
