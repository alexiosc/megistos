...
int
gallup_print (FILE * stream, const struct printf_info *info,
	      const void *const *args)
{
	...
	if (info->prec > -1) {
		switch (info->prec) {
		case 1:
			asprintf (&buffer, INT, CA.curoption + 1);
			break;
		case 2:
			asprintf (&buffer, INT, CA.fields[CA.curfield]);
			break;
		case 3:
			asprintf (&buffer, FLT,
				  CA.fields[CA.curfield] * 100.0 /
				  CA.row_t_sum);
			break;
		case 4:
			asprintf (&buffer, FLT,
				  CA.fields[CA.curfield] * 100.0 /
				  CA.row_p_sum[CA.currow]);
			break;
		case 5:
			asprintf (&buffer, FLT,
				  CA.fields[CA.curfield] * 100.0 /
				  CA.col_p_sum[CA.curcol]);
			break;

		case 10:
			asprintf (&buffer, INT, CA.row_p_sum[CA.currow]);
			break;
		case 11:
			asprintf (&buffer, FLT,
				  CA.row_p_sum[CA.currow] * 100.0 /
				  CA.row_t_sum);
			break;
		case 12:
			asprintf (&buffer, INT, CA.col_p_sum[CA.curcol]);
			break;
		case 13:
			asprintf (&buffer, FLT,
				  CA.col_p_sum[CA.curcol] * 100.0 /
				  CA.col_t_sum);
			break;
		case 14:
			asprintf (&buffer, INT, CA.row_t_sum);
			break;
		case 15:
			asprintf (&buffer, FLT,
				  CA.row_t_sum * 100.0 / CA.col_t_sum);
			break;
		case 16:
			asprintf (&buffer, INT, CA.col_t_sum);
			break;
		case 17:
			asprintf (&buffer, FLT,
				  CA.col_t_sum * 100.0 / CA.row_t_sum);
			break;
		...
		case 60:
			asprintf (&buffer, INT, CA.fld_p_sum[CA.curoption]);
			break;
		case 61:
			asprintf (&buffer, FLT,
				  CA.fields[CA.curfield] * 100.0 /
				  CA.fld_p_sum[CA.curoption]);
			break;
		case 62:
			if (!insert_color_dir)
				asprintf (&buffer, FLT,
		...
		case 80:
			CA.curfield = 0;
		case 81:
			if (ages[CA.curfield] == 0 || ages[CA.curfield] == 999)
				asprintf (&buffer, "%s", "  ");
			else
				asprintf (&buffer, "%i", ages[CA.curfield]);
			CA.curfield++;
			break;
		...
		case 85:
			if (!insert_color_dir)
				asprintf (&buffer, "%c",
					  qcomch (curquest)[CA.comboidx]);
			else
				asprintf (&buffer, "[3%im%c",
					  CA.comboidx % 7 + 1,
					  qcomch (curquest)[CA.comboidx]);
			break;
		case 86:
	...
	len =
	    fprintf (stream, "%*s", info->left ? -info->width : info->width,
		     buffer);
...
int
gallup_print_arginfo (const struct printf_info *info, size_t n, int *argtypes)
{
	return 0;
}
...
void
load_ca (int copt, int cfld, int crow, int ccol)
{
	CA.curoption = copt;
	CA.curfield = cfld;
	CA.currow = crow;
...
char   *
strndup (char *s, int c, size_t size)
{
	memset (s, c, size);
	s[size] = '\0';
...
void
alloc_ca (int size, int elem)
{
	int     i;

	CA.fields = malloc (size * elem * sizeof (int));
	for (i = 0; i < size * elem; i++)
		CA.fields[i] = 0;
...
void
gather_cax (int rows, int cols, int fields, int fcount)
{
	int     i, j;
