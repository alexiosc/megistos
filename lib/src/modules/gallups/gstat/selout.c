...
void
dump_select_options_sex_age (void)
{
	int     i, j, n, jl, jll;
	treekey_t key;
...
chart_select_options_sex (void)
{
	...
	for (i = 0; i < qseldatacnt (curquest); i++) {
		...
		for (jj = 0; jj < jl; jj++) {
			...
			j = floor ((WIDTH * CA.fields[jl * i + jj] /
				    CA.row_t_sum) + 0.5);
			for (; j > 0; j--, n--)
				strcat (tempstr, BARCHAR);
		}
...
/* there might be a differnce to the actual size of the bar beacuse of the
   ...
   ANW */
		...
		if (WIDTH - n < (WIDTH * CA.row_p_sum[i] / CA.row_t_sum)) {
			...
			memmove (tempstr + strlen (colstr), tempstr,
				 strlen (tempstr) + 1);
...
chart_select_options_age (void)
{
	...
	for (i = 0; i < qseldatacnt (curquest); i++) {
		...
		for (j = 0; j < jl; j++) {
			...
			strcat (tempstr, colstr);
			jj = floor ((WIDTH * CA.fields[i * jl + j] /
				     CA.row_t_sum) + 0.5);
			for (; jj > 0; jj--, n--)
				strcat (tempstr, BARCHAR);
		}
...
void
chart_select_options_sex_age (void)
{
