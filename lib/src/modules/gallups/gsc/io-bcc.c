 ...
 *
 * $Log$
 * Revision 1.1  2001/04/16 14:56:47  alexios
 * Initial revision
 *
...
//	sprintf(filename, "%s/%s/DATA", GALLUPSDIR, fn);
	...
	if(fread(ginfo, sizeof(struct gallup), 1, fp)!=1)return 0;

	questions = (struct question *)malloc(sizeof(struct question) * gnumq(ginfo));

	if(gflgs(ginfo) & GF_QUIZ) {
		answers = (struct answer *)malloc(sizeof(struct answer) * gnumq(ginfo));
	}
...
int _savegallup(char *filename)
{
  ...
  struct answer *a;
	...
	if(fwrite(ginfo, sizeof(struct gallup), 1, fp)!=1)return 0;
...
void freegallup(void)
{
  ...
  struct answer *a;
	...
	if(questions) {
		for(i=0;i<gnumq(ginfo);i++) {
			q = &questions[i];
			if(answers)a=&answers[i];else a=NULL;
