...
void initialize_screen()
{
  ...
  if (screen_cols == 0) screen_cols = DEFAULT_COLS;
  if (screen_rows == 0) screen_rows = DEFAULT_ROWS;
  clear_screen();
...
{
  print("\e[%d;%dH",row,col);fflush(stdout);
  current_row = row;
  current_col = col;
}
...
void display_char(int c)
{
  ...
  if (++current_col > screen_cols) current_col = screen_cols;
}
...
static int wait_for_char (struct timeval *timeout)
{
  int nfds, status;
  fd_set readfds;
  struct timeval tv;
