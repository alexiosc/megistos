...
void erase_window (zword_t w)
{
  ...
  if (h_type > V4) move_cursor (1, 1);
  else move_cursor (screen_rows, 1);
}
...
static void pad_line (int column)
{
  ...
  for (i = status_pos; i < column; i++) write_char (' ');
  status_pos = column;
}
...
void print_window (int argc, zword_t *argv)
{
  ...
  for (height = 0; height < argv[2]; height++) {
    ...
    if (height != (argv[2] - 1)) move_cursor (++row, column);
  }
}
