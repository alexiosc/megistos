...
rsys_modstats()
{
  ...
  for(pass=0;pass<2;pass++){
    ...
    while(!feof(fp)){
      ...
      cp=&lin[ptr];
      cp[len]=0;
      if(pass)strncpy(name,cp,sizeof(name));
      cp+=len+1;
	...
	if(!pass){
	  ...
	  prompt(RSMODST2,name,
		 creds,(float)(100.0*(float)((float)creds/(float)tcreds)),
		 mins,(float)(100.0*(float)((float)mins/(float)tmins)));
	}
      }
    }
...
rsys_systats()
{
  prompt(RSSYSTATS,
	 ...
	 sysvar->sigmessages,sysvar->clubmsgslive,
	 sysvar->emessages,sysvar->emsgslive,
...
rsys_linstats()
{
  ...
  char hostname[256]={0};
  char version[256]={0};
  char cpu[80]={0}, cpu_vendor[80]={0}, bogomips[80]={0};
  int  uptime;
  char load[80]={0};
  int  m[9];
  ...
  while(!feof(fp)){
    ...
    if(sscanf(line,"%s : %n\n",label,&n)==1){
      data=&line[n];
      if(sameas("MODEL",label))strcpy(cpu,data);
      else if(sameas("VENDOR_ID",label))strcpy(cpu_vendor,data);
      else if(sameas("BOGOMIPS",label))strcpy(bogomips,data);
    }
  }
  fclose(fp);
