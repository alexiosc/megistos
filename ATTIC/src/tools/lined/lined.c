...
/*
 * $Id: lined.c,v 2.0 2004/09/13 19:44:54 alexios Exp $
 *
 * $Log: lined.c,v $
		...
		default:
			if (cp + 1 + numbytes >= maxsize) {
				...
				if (inp_buffer[cp] == 32)
					...
					strcpy (wraparound, &inp_buffer[cp]);
				for (; i; i--)
					write (0, &j, 1);
				j = '\n';
				write (0, &j, 1);
...
insert ()
{
	...
	if (!get_number (&linenum, INSERT, 1, numlines, INVLIN, 0, 0))
		return 0;
...
sizewarn ()
{
	if (maxsize - numbytes < maxlen)
		prompt (SIZEWARN, maxsize - numbytes);
}
...
retype ()
{
	...
	if (!get_number (&linenum, RETYPE, 1, numlines, INVLIN, 0, 0))
		return;
...
getrange (int *m, int *n, int min, int max, int pr, int err)
{
	...
	for (;;) {
		if (cnc_more ()) {
			...
			if (!margc)
				continue;
			if (margc == 1 && inp_isX (margv[0]))
				return 0;
			inp_raw ();
			cnc_nxtcmd = inp_buffer;
		}
		*m = *n = 0;
		if (!strchr (cnc_nxtcmd, '-') &&
		    sscanf (cnc_nxtcmd, "%d%n", n, &i))
			*m = *n;
		else if (sscanf (cnc_nxtcmd, "%d-%d%n", m, n, &i) == 2);
		else if (sscanf (cnc_nxtcmd, "-%d%n", n, &i) == 1)
			*m = min;
		else if (sscanf (cnc_nxtcmd, "%d-%n", m, &i) == 1)
			*n = max;
		if (((*m) < min) || ((*n) < min) || ((*m) > max) ||
		    ((*n) > max) || ((*m) > (*n))) {
			prompt (err, min, max);
			cnc_end ();
			inp_buffer[0] = 0;
...
change ()
{
	...
	if (!getrange (&m, &n, 1, numlines, RWCHLN, RNGERR))
		return;
	if (n - m + 5 >= fmt_screenheight)
		prompt (RANGEOK, m, n);
	else
		showtext (m, n);
	cnc_end ();
...
delete ()
{
	...
	if (!getrange (&m, &n, 1, numlines, DWDELN, RNGERR))
		return;
	if (n - m + 5 >= fmt_screenheight)
		prompt (RANGEOK, m, n);
	else
		showtext (m, n);
	if (!get_bool (&yes, DELLIN + (n - m > 0), YORN, 0, 0))
		return;
	if (!yes)
		return;
...
newtext ()
{
	...
	if (!get_bool (&yes, ASKNEW, YORN, 0, 0))
		return;
	if (!yes)
		return;
...
abortedit ()
{
	...
	if (!get_bool (&yes, ABORT, YORN, 0, 0))
		return;
	if (yes) {
		unlink (filename);
		exit (1);
...
run ()
{
	...
	if (numlines)
		prompt (REMINDER, numlines, numbytes);
	prompt (RULER);
	for (;;) {
		if (numbytes >= maxsize) {
			prompt (TOOBIG);
