...
/*
 * $Id: classed.c,v 2.0 2004/09/13 19:44:52 alexios Exp $
 *
 * $Log: classed.c,v $
 ...
 * crash.
 *
 * Revision 1.5  2003/12/29 07:53:13  alexios
 * Ran megistos-config --oh.
 *
...
int
classcompare (const void *r1, const void *r2)
{
...
readclasses ()
{
	...
	if ((fp = fopen (mkfname (CLASSFILE), "r")) != NULL) {
		numclasses =
		    fread (classes, sizeof (classrec_t), MAXCLASS, fp);
	} else
		numclasses = 0;
	fclose (fp);
	if (!numclasses)
		memcpy (&classes[numclasses++], &classdel,
			sizeof (classrec_t));
}
...
saveclasses ()
{
	...
	if (fwrite (classes, sizeof (classrec_t), numclasses, fp) !=
	    numclasses) {
...
listclasschanges ()
{
	...
	for (i = 0; i < numclasses; i++) {
		...
		if (classes[i].nadays != -1)
			snprintf (s1, sizeof (s1), "%d", classes[i].nadays);
		if (classes[i].ardays != -1)
			snprintf (s2, sizeof (s1), "%d", classes[i].ardays);
...
listclasses ()
{
	...
	for (i = 0; i < numclasses; i++) {
		...
		if (classes[i].limpercall != -1)
			snprintf (s1, sizeof (s1), "%d",
				  classes[i].limpercall);
		if (classes[i].limperday != -1)
			snprintf (s2, sizeof (s2), "%d", classes[i].limperday);
...
addclass ()
{
	...
	if (!getclassname (name, ADDCNAME, 0, ALRDYEX, 0, 0))
		return 0;
	newclass = &classes[numclasses++];
	memset (newclass, 0, sizeof (classrec_t));
	strncpy (newclass->name, name, 10);
	if (!get_number
	    (&newclass->limpercall, ADDCQ1, -1, 1500, NUMERR, 0, 0))
		return 1;
	if (!get_number (&newclass->limperday, ADDCQ2, -1, 1500, NUMERR, 0, 0))
		return 1;
	if (!get_number
	    (&newclass->crdperday, ADDCQ3, 0, INT_MAX, NUMERR, 0, 0))
		return 1;
	if (!get_number
	    (&newclass->crdperweek, ADDCQ3A, INT_MIN, INT_MAX, NUMERR, 0, 0))
		return 1;
	if (!get_number
	    (&newclass->crdpermonth, ADDCQ3B, INT_MIN, INT_MAX, NUMERR, 0, 0))
		return 1;
	if (!getclassname (newclass->nocreds, ADDCQ4A, 1, UNKCLSS, 0, 0))
		return 1;

	if (!getclassname (newclass->credpost, ADDCQ4B, 1, UNKCLSS, 0, 0))
		return 1;

	if (!get_number
	    (&newclass->nadays, ADDCQ5, -1, 0x7fffffff, NUMERR, 0, 0))
		return 1;
	if (newclass->nadays != -1) {
		if (!getclassname
		    (newclass->notaround, ADDWHCL, 1, UNKCLSS, 0, 0))
			return 1;
	} else {
	...
	if (!get_number
	    (&newclass->ardays, ADDCQ6, -1, 0x7fffffff, NUMERR, 0, 0))
		return 1;
	if (newclass->ardays != -1) {
		if (!getclassname
		    (newclass->around, ADDWHCL, 1, UNKCLSS, 0, 0))
			return 1;
	} else {
		strcpy (newclass->around, newclass->name);
	}

	if (!get_bool (&i, ADDCQ7, BOOLERR, 0, 0))
		return 1;
	if (i)
		newclass->flags |= CLF_NOCHRGE;
	if (!get_bool (&i, ADDCQ8, BOOLERR, 0, 0))
		return 1;
	if (i)
		newclass->flags |= CLF_LINUX;
	if (!get_bool (&i, ADDCQ9, BOOLERR, 0, 0))
		return 1;
	if (i)
		newclass->flags |= CLF_LOCKOUT;
	if (!get_bool (&i, ADDCQ10, BOOLERR, 0, 0))
		return 1;
	if (!i)
		newclass->flags |= CLF_CRDXFER;

	if (!keyed (newclass->keys))
		return 1;
	return 0;
}
...
delclass ()
{
	...
	if (!getclassname (name, DELCNAME, 1, UNKCLSS, 0, 0))
		return;
	if (!get_bool (&i, DELCCNF, BOOLERR, 0, 0))
		return;
	if (!i)
		return;

	for (i = 0; i < numclasses; i++)
		if (sameas (name, classes[i].name))
			break;
	if (i + 1 < numclasses) {
		memmove (&classes[i], &classes[i + 1],
			 sizeof (classrec_t) * (numclasses - (i + 1)));
	}
	numclasses--;
	prompt (DELWARN);
}
...
listkeys (bbskey_t * keys)
{
	...
	for (i = 0; i < (32 * KEYLENGTH); i++) {
		...
		if (!j) {
			if (first != -1) {
				if (first < (i - 1))
					prompt (VIEWKY2, first + 1, i);
				if (first == (i - 1))
					prompt (VIEWKY1, first + 1);
				first = -1;
			}
		} else {
			if (first == -1)
...
viewclass ()
{
	...
	if (!getclassname (name, VEWCNAME, 1, UNKCLSS, 0, 0))
		return;
	...
	prompt (VIEWC5,
		classes[i].limpercall, msg_getunit (MINSNG,
						    classes[i].limpercall));
	prompt (VIEWC6, classes[i].limperday,
		msg_getunit (MINSNG, classes[i].limperday));
	prompt (VIEWC7, classes[i].crdperday,
		msg_getunit (CRDSNG, classes[i].crdperday));
	prompt (VIEWC7A, classes[i].crdperweek,
		msg_getunit (CRDSNG, classes[i].crdperweek));
	prompt (VIEWC7B, classes[i].crdpermonth,
		msg_getunit (CRDSNG, classes[i].crdpermonth));

	if (classes[i].flags & CLF_NOCHRGE)
		prompt (VIEWC8);
	if (classes[i].flags & CLF_LINUX)
		prompt (VIEWC9);
	if (classes[i].flags & CLF_LOCKOUT)
		prompt (VIEWC10);
	if (classes[i].flags & CLF_CRDXFER)
		prompt (VIEWC11);
...
editclass ()
{
	...
	if (!getclassname (name, EDTCNAME, 1, UNKCLSS, 0, 0))
		return;
	...
	if (!get_number
	    (&n->limpercall, ADDCQ1, -1, 1500, NUMERR, DEFINT, n->limpercall))
		return;
	if (!get_number
	    (&n->limperday, ADDCQ2, -1, 1500, NUMERR, DEFINT, n->limperday))
		return;
	if (!get_number
	    (&n->crdperday, ADDCQ3, 0, 0x7fffffff, NUMERR, DEFINT,
	     n->crdperday))
		return;
	...
	if (!get_number (&n->crdpermonth, ADDCQ3B, INT_MIN, INT_MAX, NUMERR,
			 DEFINT, n->crdpermonth))
		return;

	if (!getclassname (n->nocreds, ADDCQ4A, 1, UNKCLSS, 1, n->nocreds))
		return;
	if (!getclassname (n->credpost, ADDCQ4B, 1, UNKCLSS, 1, n->credpost))
		return;

	if (!get_number
	    (&n->nadays, ADDCQ5, -1, 0x7fffffff, NUMERR, DEFINT, n->nadays))
		return;
	if (n->nadays != -1) {
		if (!getclassname
		    (n->notaround, ADDWHCL, 1, UNKCLSS, 1, n->notaround))
			return;
	} else {
	...
	if (!get_number
	    (&n->ardays, ADDCQ6, -1, 0x7fffffff, NUMERR, DEFINT, n->ardays))
		return;
	if (n->ardays != -1) {
		if (!getclassname
		    (n->around, ADDWHCL, 1, UNKCLSS, 1, n->around))
			return;
	} else {
	...
	if (!get_bool
	    (&i, ADDCQ7, BOOLERR, DEFCHR, (n->flags & CLF_NOCHRGE) != 0))
		return;
	(i) ? (n->flags |= CLF_NOCHRGE) : (n->flags &= ~CLF_NOCHRGE);
	if (!get_bool
	    (&i, ADDCQ8, BOOLERR, DEFCHR, (n->flags & CLF_LINUX) != 0))
		return;
	(i) ? (n->flags |= CLF_LINUX) : (n->flags &= ~CLF_LINUX);
	if (!get_bool
	    (&i, ADDCQ9, BOOLERR, DEFCHR, (n->flags & CLF_LOCKOUT) != 0))
		return;
	(i) ? (n->flags |= CLF_LOCKOUT) : (n->flags &= ~CLF_LOCKOUT);
	if (!get_bool
	    (&i, ADDCQ10, BOOLERR, DEFCHR, (n->flags & CLF_CRDXFER) == 0))
		return;
...
moveclass ()
{
	...
	if (!getclassname (source, MOVSRC, -1, UNKCLSS, 0, 0))
		return;
	if (!getclassname (target, MOVTRG, 1, UNKCLSS, 0, 0))
		return;
	if (sameas (source, target)) {
		...
		if ((dp = opendir (mkfname (USRDIR))) == NULL) {
			error_fatalsys ("Unable to diropen %s",
					mkfname (ONLINEDIR));
		}
...
confirmquit ()
{
	...
	if (!get_bool (&i, QUITWRN, BOOLERR, DEFCHR, 0))
		return;
	if (i)
		exit (0);
}
...
mainmenu ()
{
	...
	for (;;) {
		...
		if (cnc_nxtcmd) {
			while (*cnc_nxtcmd && isspace (*cnc_nxtcmd))
				cnc_nxtcmd++;
			if (*cnc_nxtcmd) {
				c = cnc_chr ();
				switch (c) {
