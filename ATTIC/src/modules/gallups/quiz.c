...
store_init (char *userid)
{
	...
	if ((fp = fopen (lockfile, "r")) != NULL) {
		...
		stored_answers =
		    malloc (answer_count * sizeof (struct storerec));

		fread (&stored_answers[0], sizeof (struct storerec),
		       answer_count, fp);
	...
	answer_count++;
	stored_answers =
	    realloc (stored_answers, answer_count * sizeof (struct storerec));
...
store_done (void)
{
	...
	fwrite (&stored_answers[0], sizeof (struct storerec), answer_count,
		fp);
	fclose (fp);
}
