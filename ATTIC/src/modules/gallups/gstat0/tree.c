...
struct treenode *tree_getleaf(struct treenode *t, treekey_t key, int idx)
{
	if(idx >= key[0])return t;
	if(key[ idx+1 ] >= t->leafcount)return NULL;
...
void key_copy(treekey_t dst, treekey_t src)
{
  int i;
	...
	for(i=0;i<GMAXCLASS+1;i++)dst[i] = src[i];
}
