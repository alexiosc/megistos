...
/*
 * $Id: reply.c,v 2.0 2004/09/13 19:44:50 alexios Exp $
 *
 * $Log: reply.c,v $
...
quotemessage (message_t *msg, char *quotefn)
{
	...
	if (ecuser.prefs & ECP_QUOTEASK) {
		if (!get_bool (&yes, QMASK, QMERR, 0, 0))
			return 0;
	} else
		yes = (ecuser.prefs & ECP_QUOTEYES) != 0;
...
reply (message_t *org, int forceemail)
{
	...
	for (; !nomore;) {
		...
		if (!numcopies) {
			...
			if (key_owns (&thisuseracc, attkey) &&
			    usr_canpay (attchg)) {
				for (;;) {
					if (!askyesno
			...
			if (rrr)
				msg.flags |= MSF_RECEIPT;
			sprintf (tmp, HST_CC " %s/%d", EMAILCLUBNAME,
				 original);
		...
		if ((fp = fopen (header, "w")) == NULL) {
			error_fatalsys ("Unable to create message header %s",
					header);
		}
		if (fwrite (&msg, sizeof (msg), 1, fp) != 1) {
			...
			fclose (fp);
			errno = i;
			error_fatalsys ("Unable to write message header %s",
					header);
		}
		fclose (fp);

		if (!attfile[0])
			sprintf (command, "%s %s %s", mkfname (BBSMAILBIN),
				 header, body);
		else {
		...
		if ((fp = fopen (header, "r")) == NULL) {
			error_fatalsys ("Unable to read message header %s",
					header);
		}
		...
		if (checkmsg.msgno) {
			if (!numcopies) {
				prompt (WECONFS,
					msg.club[0] ? clubhdr.
					club : EMAILCLUBNAME, checkmsg.msgno);
				org->replies++;
				writemsgheader (org);
			} else
				prompt (WECONFC, original, checkmsg.msgno,
					checkmsg.to);
			...
			if (usr_insys (checkmsg.to, 1)) {
				if (!msg.club[0]) {
					sprompt_other (othrshm, out_buffer,
						       numcopies ? WERNOTC :
						       WERNOT, checkmsg.from,
						       checkmsg.subject);
				} else {
					sprompt_other (othrshm, out_buffer,
						       WCRNOT, checkmsg.from,
						       checkmsg.club,
						       checkmsg.subject);
				}
		...
		usr_chargecredits (msg.club[0] ? clubhdr.
				   postchg : (net ? wrtchg + netchg : wrtchg));
