...
/*
 * $Id: distlist.c,v 2.0 2004/09/13 19:44:50 alexios Exp $
 *
 * $Log: distlist.c,v $
...
checkselectname (char *name)
{
	...
	if ((dp = opendir (MSGSDISTDIR)) == NULL)
		return 0;
...
confdistlist ()
{
	...
	for (;;) {
		if (!dlgetmenu
		    (&opt, firsttime, sop ? DLSMNU : DLMNU,
		     sop ? DLSMNUS : DLMNUS, DLERRS, sop ? "HLADPSK" : "HLAD",
		     0, 0))
			break;
		firsttime = 0;
		switch (opt) {
...
opendistribution (char *dist)
{
	...
	if ((dist[0] == '!' && dist[1] == '!') || sameas (dist, "!all")) {
		...
		initlist ();
		inspoint =
		    scandir (mkfname (USRDIR), &d, usrselect, ncsalphasort);
...
readdistribution ()
{
	if (current[0] == '!') {
		...
		for (; readpoint < inspoint;) {
			...
			if (sameas (uacc.curclss, &current[1]))
				return uid;
		}
		return NULL;
	}
