...
/*
 * $Id: cleanup.c,v 2.0 2004/09/13 19:44:51 alexios Exp $
 *
 * $Log: cleanup.c,v $
	...
	for (i = 0; i < numtops - 1; i++) {
		if (f->downloaded > topfiles[i].downloads) {
			for (j = numtops - 1; j > i; j--) {
				memcpy (&topfiles[j], &topfiles[j - 1],
					sizeof (struct topfiles));
			}
			sprintf (topfiles[i].text,
				 "%9d %-22.22s %-10.10s %.35s", f->downloaded,
				 f->fname, f->uploader, f->summary);
...
topfiles_footer (FILE * fp, struct libidx *l)
{
	...
	if (fp == NULL)
		return;
	fprintf (fp,
		 "Downloads Filename               Uploader   Description\n"
		 "---------------------------------------------------"
		 "----------------------------\n");
	...
	for (i = 0; i < nummtps - 1; i++) {
		if (f->downloaded > mastertopfiles[i].downloads) {
			...
			if (strlen (tmp) > 30)
				...
				strcpy (fullname, tmp);
			sprintf (mastertopfiles[i].text,
				 "%5d %-30.30s %-10.10s %.30s", f->downloaded,
				 fullname, f->uploader, f->summary);
...
mastertopfiles_footer (FILE * fp, struct libidx *l)
{
	...
	fprintf (fp,
		 "Dnlds Library/Filename               Uploader   Description\n"
		 "---------------------------------------------------"
		 "----------------------------\n");
...
oslib (struct libidx *l)
{
	...
	printf ("%-37.37s %5d %3d.%02d (Stats N/A, OS-only library)\n",
		l->fullname, files, bytes >> 20, frac (bytes));
}
...
libcleanup ()
{
	...
	if (libread (libmain, 0, &mainlibrary) != 1) {
		printf
		    ("Unable to find Main (\"%s\") library! Can't continue.\n",
		     libmain);
		return;
	}
	...
	while (more) {
		...
		if (strlen (fullname) > 37)
			sprintf (fullname, "...%s",
				 &(l.fullname[strlen (l.fullname) - 34]));
...
get_dayssince ()
{
	...
	strcpy (fname, mkfname (FILELIBDIR "/.LASTCLEANUP"));
...
cleanup ()
{
	if ((dayssince = get_dayssince ()) < 1) {
		printf
		    ("filecleanup: No need to cleanup file libraries yet.\n");
		return 0;
	}
