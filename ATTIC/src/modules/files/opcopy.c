...
/*
 * $Id: opcopy.c,v 2.0 2004/09/13 19:44:51 alexios Exp $
 *
 * $Log: opcopy.c,v $
...
static
    int
docopy (struct fileidx *f, struct libidx *target)
{
...
op_copy ()
{
	...
	if ((spec = getfilenames (OCPYASK)) == NULL)
		return;
	...
	while (fr) {
		...
		for (;;) {
			...
			if (!all) {
				...
				res =
				    get_menu (&c, 0, 0, OCPYMNU, 0, "YNA", 0,
					      0);
