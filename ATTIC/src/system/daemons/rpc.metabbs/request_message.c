...
/*
 * $Id: request_message.c,v 2.0 2004/09/13 19:44:53 alexios Exp $
 *
 * $Log: request_message.c,v $
...
#ifndef RCS_VER 
#define RCS_VER "$Id: request_message.c,v 2.0 2004/09/13 19:44:53 alexios Exp $"
...
static void try_compress(char **source, int *len)
{
  char *tmp;
  unsigned long outlen;

  /* Allocating much more than buffer required by zlib. Surely, it's not going
     to allow a bytestring to *grow* by 10% instead of shrinking? :-) */

  tmp=malloc(outlen=(*len + (*len/10) + 12));
  if(tmp==NULL)return;		/* Failed, let's return. Zlib can read
...
struct club_message *
distclub_request_message_1_svc(message_request_t *msgreq, struct svc_req *req)
{
...
#ifdef DEBUG
  fprintf(stderr,"Msg request for %s/%d by BBS %s from BBS %s (cmp=%d)",
	  msgreq->targetclub,msgreq->msgno,
	  msgreq->codename,msgreq->targetname,msgreq->compression);
#endif
...
#error "mkfname() not available here, but function obsoleted anyway"
  ...
  retstuff.club_message_u.message.message.message_len=st.st_size-sizeof(msg);
  retstuff.club_message_u.message.message.message_val=malloc(st.st_size-sizeof(msg));
  ...
  while(i<retstuff.club_message_u.message.message.message_len){
    ...
    bytes_read=
      read(fd,
	   &retstuff.club_message_u.message.message.message_val[i],
	   4096);
...
#error "mkfname() not defined here, but function obsoleted anyway"
    ...
    i=0;
    while(i<retstuff.club_message_u.message.attachment.attachment_len){
      int bytes_read;
      bytes_read=
	read(fd,
	     &retstuff.club_message_u.message.attachment.attachment_val[i],
	     4096);
    ...
    if(i<retstuff.club_message_u.message.attachment.attachment_len){
      retstuff.result_code=CLR_CORRUPTMSG;
      return &retstuff;
    }
  ...
  if(msgreq->compression){
    ...
    retstuff.club_message_u.message.comp_result.compr_u.compr.orig_msg_len=
      retstuff.club_message_u.message.message.message_len;
    retstuff.club_message_u.message.comp_result.compr_u.compr.orig_att_len=
      retstuff.club_message_u.message.attachment.attachment_len;


    /* And now do the actual compression */

    try_compress(&retstuff.club_message_u.message.message.message_val,
		 &retstuff.club_message_u.message.message.message_len);
    try_compress(&retstuff.club_message_u.message.attachment.attachment_val,
		 &retstuff.club_message_u.message.attachment.attachment_len);
  }
#endif HAVE_ZLIB
