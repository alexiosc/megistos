...
/*
 * $Id: chat.c,v 2.0 2004/09/13 19:44:53 alexios Exp $
 *
 * $Log: chat.c,v $
...
static const char rcsinfo[] = "$Id: chat.c,v 2.0 2004/09/13 19:44:53 alexios Exp $";
...
send (char *s)
{
	...
	while ((ch = *s++)) {
		if (ch == '\\') {
			...
			case 'p':	/* '\p' == pause */
				debug (D_CHAT,
				       "\\p specified, pausing for 1s.");
				sleep (1);
				s++;
				continue;
			case 'd':	/* '\d' == delay */
				debug (D_CHAT,
				       "\\d specified, pausing for 2s.");
				sleep (2);
				s++;
				continue;
			case 'K':	/* '\K' == BREAK */
				debug (D_CHAT,
				       "\\K specified, sending BREAK.");
...
expmatch (char *got, char *exp)
{
	...
	while (*exp) {
		...
		if (*((unsigned char *) exp) == AUTOBD) {
			...
			if (*got == '\0')
				return 0;	/* didn't get it all yet */
			autorate[ptr] = '\0';
			exp++;
			continue;
		}
