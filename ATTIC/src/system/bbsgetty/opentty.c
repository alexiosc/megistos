...
/*
 * $Id: opentty.c,v 2.0 2004/09/13 19:44:53 alexios Exp $
 *
 * $Log: opentty.c,v $
...
static const char rcsinfo[] = "$Id: opentty.c,v 2.0 2004/09/13 19:44:53 alexios Exp $";
...
opentty ()
{
	...
	if (localline) {
		debug (D_RUN,
		       "The line is local, initiating connection immediately.");
	} else
		writelinestatus (LSR_RING);


	/* Open the device */
	if ((fd =
	     open (devname, waitfor ? (O_RDWR | O_NDELAY) : (O_RDWR))) < 0) {
	...
	if (delay) {
		debug (D_RUN, "DELAY specified, sleeping for %d seconds...",
		       delay);
		sleep (delay);
	...
	if (connectstr) {
		...
		if (!strcmp (connectstr, "DEFAULT"))
			connectstr = DEF_CONNECT;
		...
		if (!lockedbaud) {
			if (autobaud) {
				...
				if ((nspeed = atoi (autorate)) > 0) {
					...
					if (strlen (speed)) {
						debug (D_RUN,
						       "Modem reports %d bps, selected CBAUD is %s bps.",
						       nspeed, speed);
					} else {
						debug (D_RUN,
						       "Modem reports %d bps, yet no CBAUD flag was found.");
					}
				}
			}
		}
		...
		if (strlen (autorate) && atoi (autorate) > 0) {
			debug (D_RUN,
			       "Will report %d bps (actual comms speed) to the BBS.",
			       atoi (autorate));
