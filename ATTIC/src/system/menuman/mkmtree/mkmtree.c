...
/*
 * $Id: mkmtree.c,v 2.0 2004/09/13 19:44:54 alexios Exp $
 *
 * $Log: mkmtree.c,v $
...
addsymbol (char *s)
{
	if (!checksymbol (s)) {
		symtab =
		    realloc (symtab, (++symtabentries) * (PAGENAMELEN + 1));
		strncpy (&symtab[(symtabentries - 1) * (PAGENAMELEN + 1)], s,
			 16);
	}
}
...
marksymbol (char *s)
{
	...
	for (i = 0; i < symtabentries; i++) {
		if (sameas (s, &symtab[i * (PAGENAMELEN + 1)])) {
			char   *c =
			    &symtab[i * (PAGENAMELEN + 1) + PAGENAMELEN];
			if (*c)
				return 1;
			(*c)++;
			return 0;
...
parse ()
{
	int     mode = 0, linenum = 0, pagetype = 0, key = 0, creds =
	    DEFAULTCCR;
	...
	while (!feof (in)) {
		...
		*sp = 0;
		...
		for (directive = 0, i = 0; !directive && directives[i].name[0];
		     i++) {
			if (sameas (directives[i].name, keyword))
				directive = directives[i].code;
		}
		if (!directive) {
		...
		case 0:
			...
			default:
				error
				    ("Expecting PAGE directive, found \"%s\".\n",
				     linenum, keyword);
				exit (-1);
			}
			break;
		case 1:
			...
			case DIR_DESC:
				if (strlen (value) > PAGEDESCRLEN - 1) {
					error
					    ("Page description '%s' too long.\n",
					     linenum, value);
					exit (-1);
				} else if (numdescs > NUMLANGUAGES) {
					error
					    ("More than %d description strings.\n",
					     linenum, NUMLANGUAGES);
					exit (-1);
				} else {
					int     i;

					for (i = numdescs; i < NUMLANGUAGES;
					     i++) {
						strncpy (descr[i], value,
							 PAGEDESCRLEN - 1);
					...
					if (value2 == NULL) {
						...
						*(value2++) = 0;
						...
						if (strlen (input) >
						    INPUTSTRLEN - 1) {
							error
							    ("Input string '%s' too long.\n",
							     linenum, value2);
							exit (-1);
						}
						strncpy (input, value2,
							 INPUTSTRLEN - 1);
						input[INPUTSTRLEN - 1] = 0;
					}
					if (strlen (value) > FNAMELEN - 1) {
						error
						    ("Module name '%s' too long.\n",
						     linenum, value);
						exit (-1);
					}
			...
			case DIR_OPT:
				{
					...
					if (value2 == NULL) {
						error
						    ("Expecting two parameters for OPT, found '%s'.\n",
						     linenum, value);
						exit (-1);
					} else {
						*(value2++) = 0;
						...
						if (strlen (value) != 1) {
							error
							    ("Expecting character as first parameter, found '%s'.\n",
							     linenum, value);
							exit (-1);
						} else if (!isprint (value[0])) {
							error
							    ("Expecting printable character for OPT, found '%s'\n",
							     linenum, value);
							exit (-1);
						}
			...
			case DIR_END:
				...
				if (!numopts && pagetype == 'M') {
					error
					    ("Menu page %s without any options!\n",
					     linenum, name);
					exit (-1);
				} else if (numopts && pagetype != 'M') {
					error
					    ("Menu options defined for non-menu page %s!\n",
					     linenum, name);
					exit (-1);
				} else if (file[0] && pagetype != 'M' &&
					   pagetype != 'F') {
					error
					    ("Can't use the file directive in page %s!\n",
					     linenum, name);
					exit (-1);
				} else if (exec[0] && pagetype != 'E') {
					error
					    ("Can't use the exec directive in page %s!\n",
					     linenum, name);
					exit (-1);
				} else if (run[0] && pagetype != 'R') {
					error
					    ("Can't use the run directive in page %s!\n",
					     linenum, name);
					exit (-1);
				} else if (!file[0] &&
					   (pagetype == 'M' ||
					    pagetype == 'F')) {
					error
					    ("Need a file directive for page %s!\n",
					     linenum, name);
					exit (-1);
				}
				...
				case 'M':
					...
					qsort (opts, numopts,
					       sizeof (struct menuoption),
					       optcmp);
					memcpy (pagerec.m.opts, opts,
						sizeof (pagerec.m.opts));
					break;
				case 'F':
					memset (&pagerec, 0, sizeof (pagerec));
					strcpy (pagerec.f.name, name);
					memcpy (&pagerec.f.descr, descr,
						sizeof (descr));
				...
				if (fwrite (&pagerec, sizeof (pagerec), 1, out)
				    != 1) {
					fprintf (stderr,
						 "mkmtree: Unable to write %s\n",
						 MENUMANPAGES);
					exit (-1);
				}
				file[0] = 0;
...

	{
		...
		for (i = 0; i < symtabentries; i++) {
			...
			if (!*c) {
				fprintf (stderr,
					 "mkmtree: line %d: Undefined page reference '%s'.\n",
					 linenum, s);
				exit (-1);
			}
		}
...
loadindex ()
{
	...
	while (!feof (fp)) {
		...
		if (sscanf (line, "%s %ld", page, &offset) != 2) {
			fclose (fp);
			fprintf (stderr,
				 "mkmtree: Menuman index %s bad format\n",
				 MENUMANINDEX);
			exit (-1);
		}
		numpages++;
